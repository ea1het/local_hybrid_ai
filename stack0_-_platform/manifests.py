    ...
    for stack_id, data in manifests.items():
        ...
        for resource in data["owns"]:
            ...
            if previous is not None:
                fail(f"ownership collision: {resource} is owned by stack{previous} and stack{stack_id}")
            owner_of[resource] = stack_id
    for stack_id, data in manifests.items():
        required = dependency_closure(manifests, dependencies(data, target), target)
        optional = dependency_closure(manifests, data["optional"], target)
...
def main() -> None:
    ...
    manifests = load_manifests(args.root.resolve())
    if args.command == "validate":
        validate_graph(manifests, args.target)
        validate_contracts(manifests, args.target)
        print(f"manifest graph ({'target' if args.target else 'current'}): OK ({len(manifests)} stacks)")
        return
    for target in (False, True):
        validate_graph(manifests, target)
        validate_contracts(manifests, target)
    ...
    if args.command == "list":
        if args.json:
            ...
            for stack_id in sorted(manifests):
                ...
                print(f"stack{stack_id}: {data['directory']} [{atomic}] [recovery:{recovery_mode}]")
