...
def _materialize_source(commit: str, destination: Path) -> None:
    ...
    try:
        cp = subprocess.run(["git", "archive", "--format=tar", "--output", str(tar_path), commit], cwd=dr_restore_all.PROJECT_ROOT, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True, check=False)
...
def _prepare_stage_context(backup_set: Path, destination: Path) -> StageContext:
    plan = dr_restore_all.plan_restore_all(backup_set); metadata = dr_restore_all.read_completed_backup_set(backup_set); _ensure_private_empty_destination(destination)
...
def _verify_external_config(context: StageContext, values: dict[str, str]) -> int:
    manifests = dr.load_manifests(); resources = {(sid, resource["id"]): resource for sid in context.resolved_stacks for resource in manifests[sid].get("recovery", {}).get("resources", [])}; verified = 0
    for prereq in context.metadata.get("prerequisites", []):
        ...
        dr.require_env_value(values, key, label=f"recovery prerequisite {prereq['resource_id']}"); verified += 1
    return verified
