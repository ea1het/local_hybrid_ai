...
from commands import component_state,install,stack_contracts,upgrade
SCHEMA_VERSION="3"
...
def inventory(*,runtime_root:Path|None=None,deployed_versions:dict[str,str]|None=None):
 env=upgrade.read_env();records=upgrade.component_records();deployed_versions=deployed_versions if deployed_versions is not None else component_state.deployed_versions(runtime_root or upgrade.runtime_root());rows=[]
...
def stack_inventory(component_rows=None):
 ...
 for sid in sorted(manifests):
  manifest=manifests[sid];entry=lifecycle["stacks"][str(sid)];runtime=install.stack_state(manifest);state,health=_runtime_summary(entry,runtime);key=f"stack{sid}";owned=[r for r in component_rows if r["stack"]==key]
  rows.append(stack_contracts.json_payload(sid,manifest["directory"],name=_stack_name(manifest["directory"]),state=state,health=health,drift=component_state.aggregate_drift(owned)))
 return rows
def json_payload():
 try:
  components=inventory();return {"schema_version":SCHEMA_VERSION,"command":"status","success":True,"stacks":stack_inventory(components),"components":components}
 except (component_state.ComponentStateError,install.InstallerError,stack_contracts.StackContractError,StatusError,OSError,json.JSONDecodeError) as exc:return {"schema_version":SCHEMA_VERSION,"command":"status","success":False,"error":{"code":"STATUS_STATE_INVALID","message":str(exc)}}
