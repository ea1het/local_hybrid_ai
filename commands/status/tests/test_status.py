...
"""Operational-status regression tests."""
...
class StatusOperationalTests(unittest.TestCase):
 ...
 def test_json_status_contains_no_upgrade_state(self):
  ...
  with mock.patch("commands.status.stack_inventory",return_value=stacks):payload=status.json_payload()
 ...
 def test_status_does_not_depend_on_upgrade_inventory_or_registry(self):
  source=open(status.__file__,encoding="utf-8").read()
  for forbidden in ("read_env(","component_records(","load_catalog(","compose_image(","running_image(","resolve_identity(","deployed_versions(","aggregate_drift(","upgrade_registry"):
