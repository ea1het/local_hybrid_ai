...
def _runtime_running(sid,lifecycle):
 required=_required_containers(sid,lifecycle);return bool(required) and all(installer.is_running(installer.container_state(n)) for n in required)
...
def _resolve_one(selector,manifests):
 try:values=installer.resolve_requested([selector],manifests)
 except installer.InstallerError as exc:raise RuntimeLifecycleError("STACK_UNKNOWN",str(exc)) from exc
...
def _preflight():
 ...
 manifests=installer.all_manifests();lifecycle=installer.load_lifecycle();installer.validate_registry(manifests,lifecycle)
 try:installer.run(["docker","compose","version"],capture=True)
...
def execute(action,selector):
 ...
 if not installer.stack_prepared(directory):raise RuntimeLifecycleError("STACK_NOT_PREPARED",f"stack{sid} is not PREPARED")
 if action=="stop":
 ...
 cp=installer.run(["docker","compose",action],cwd=cwd,capture=True,check=False)
 if cp.returncode!=0:
 ...
 if action=="start":
  try:installer.wait_required_runtime(sid,lifecycle["stacks"][str(sid)])
  except installer.InstallerError as exc:raise RuntimeLifecycleError("STACK_START_NOT_READY",str(exc)) from exc
...
def json_payload(action,selector):
 ...
 except (RuntimeLifecycleError,installer.InstallerError,stack_contracts.StackContractError) as exc:
