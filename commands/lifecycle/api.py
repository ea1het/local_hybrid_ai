...
ROOT=Path(__file__).resolve().parents[1];SCHEMA_VERSION="1"
...
class RuntimeResult:
 ...
 def as_dict(self):
  return stack_contracts.json_payload(self.stack_id,self.directory,schema_version=SCHEMA_VERSION,command=f"runtime.{self.action}",success=True,directory=self.directory,containers=list(self.containers))
...
def json_payload(action,selector):
 """Execute the domain operation and return only JSON-compatible data."""
 try:return execute(action,selector).as_dict()
  ...
  code=exc.code if isinstance(exc,RuntimeLifecycleError) else "RUNTIME_INTERNAL_ERROR";message=exc.message if isinstance(exc,RuntimeLifecycleError) else str(exc)
  return {"schema_version":SCHEMA_VERSION,"command":f"runtime.{action}","success":False,"error":{"code":code,"message":message}}
