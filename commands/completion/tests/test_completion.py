...
class CompletionTests(unittest.TestCase):
 ...
 def test_stack_completion_is_manifest_driven_and_numeric(self):
  with mock.patch("commands.completion.api.install.all_manifests",return_value={0:{},2:{},7:{}}):self.assertEqual([v for v in completion.complete(["start",""]) if not v.startswith("--")],["0","2","7"]);self.assertEqual([v for v in completion.complete(["stop",""]) if not v.startswith("--")],["0","2","7"]);self.assertEqual(completion.complete(["start","stack"]),[])
 def test_install_completion_exposes_numeric_stacks_and_all_public_options(self):
  with mock.patch("commands.completion.api.install.all_manifests",return_value={0:{},7:{}}):values=completion.complete(["install",""])
 ...
 def test_restore_leaf_completion_exposes_public_options(self):
  cases={("restore","list-backup-sets",""):{"--backup-root"},("restore","drill","/backup",""):{"--destination"},("restore","apply","/backup",""):{"--check-clean-target","--execute","--confirm-clean-target","--memory-sync-ssh-bootstrap"},("restore","resume","/backup",""):{"--memory-sync-ssh-bootstrap"}}
 ...
 def test_upgrade_components_are_manifest_driven_with_numeric_public_stack(self):
  ...
  with mock.patch("commands.completion.api.component_inventory.compile_upgrade_catalog",return_value=compiled),mock.patch("commands.completion.api.install.all_manifests",return_value={2:{}}):self.assertEqual([v for v in completion.complete(["upgrade","2",""]) if not v.startswith("--")],["redis","searxng"]);self.assertEqual([v for v in completion.complete(["upgrade","stack2",""]) if not v.startswith("--")],[])
 def test_upgrade_actions_match_public_grammar(self):
  compiled={"schema_version":1,"stacks":[{"id":"stack7","components":[{"id":"open-webui"}]}]}
  with mock.patch("commands.completion.api.component_inventory.compile_upgrade_catalog",return_value=compiled),mock.patch("commands.completion.api.install.all_manifests",return_value={7:{}}):self.assertEqual([v for v in completion.complete(["upgrade","7","open-webui",""]) if not v.startswith("--")],["clear","select"]);self.assertEqual([v for v in completion.complete(["upgrade","policy","7","open-webui",""]) if not v.startswith("--")],["clear","set"])
 def test_global_flags_are_available_at_nested_leaves(self):
  for words in (["restore","plan","/backup",""] ,["inventory","rescan",""] ,["upgrade","check",""]):
   with self.subTest(words=words):values=completion.complete(list(words));self.assertIn("--json",values);self.assertIn("--yes",values)
 ...
 def test_private_endpoint_fails_quiet(self):
  with mock.patch("commands.completion.api.complete",side_effect=RuntimeError("boom")):self.assertEqual(cli.main(["__complete","upgrade",""]),0)
 ...
 def test_completion_install_fails_closed_without_global_yes(self):
  with mock.patch("commands.completion.api.install_completion") as install:rc=cli.main(["completion","install"])
  self.assertEqual(rc,2);install.assert_not_called()
 def test_completion_install_uses_global_yes(self):
  with mock.patch("commands.completion.api.install_completion",return_value=("bash",Path("/tmp/local-ai"))) as install,mock.patch("commands.cli.render.render_cli"):rc=cli.main(["completion","install","--yes"])
