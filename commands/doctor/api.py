...
SCHEMA_VERSION="1";ROOT=Path(__file__).resolve().parents[2]
...
def run_checks():
 ...
 try:manifests=installer.all_manifests();lifecycle=installer.load_lifecycle();installer.validate_registry(manifests,lifecycle)
 except (installer.InstallerError,OSError,subprocess.CalledProcessError) as exc:checks.append(_check("lifecycle_registry","fail","manifest/lifecycle registry validation failed",error=str(exc)))
 ...
 except (component_inventory.InventoryError,OSError,subprocess.CalledProcessError) as exc:checks.append(_check("component_inventory","fail","manifest component inventory validation failed",error=str(exc)))
