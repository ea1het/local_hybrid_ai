...
def build_payload(argv,*,assume_yes=False):
 try:
  args=_parser().parse_args(argv);_public_stack_ids(args.stacks);execute_mode=not args.plan and not args.dry_run;installer.preflight(execute_mode);manifests=installer.all_manifests();lifecycle=installer.load_lifecycle();installer.validate_registry(manifests,lifecycle);requested=installer.resolve_requested(args.stacks,manifests);plan=installer.resolve_plan(args.stacks,args.target);actions,reconcile_ids,changed_stack_ids=installer.build_actions(requested,plan,manifests,lifecycle,force_reconcile=args.reconcile)
 except ValueError as exc:return _error("CLI_USAGE",str(exc)),2
 except (installer.InstallerError,OSError,subprocess.CalledProcessError) as exc:return _error("INSTALL_ERROR",str(exc)),1
 ...
 cp=subprocess.run([sys.executable,str(installer.ROOT/"commands"/"installer.py"),*argv,"--yes"],cwd=installer.ROOT,text=True,capture_output=True,check=False)
