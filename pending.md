...
- **Generic `restore all` execution.** The read-only manifest-driven planner is host-qualified against the canonical recovery point. The next bounded executor slice now exists in `bkp-dr/dr_restore_stage.py` with CLI `bkp-dr/restore-stage.py`: it materializes the recorded Git commit, restores protected `operational.env` with mode `0600`, verifies `REQUIRE`/`external-config` prerequisites and restores `pre-prepare` archive resources into an isolated runtime tree. Host-qualify this staging executor, then extend strategy-driven execution into isolated PostgreSQL/Gitea services and lifecycle PREPARE/DEPLOY/VERIFY. Do not enable live-host mutation before clean-target qualification.
...
The generic read-only `restore all` planner is implemented and host-qualified against the canonical full recovery point. It verified five checksummed files, reconstructed the exact phase order, failed closed when real execution was attempted, left the installer converged and runtime unchanged, and ended with a clean Git worktree.
