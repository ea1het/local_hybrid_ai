...
- **Generic `restore all` execution.** The read-only manifest-driven planner/preflight now exists in `bkp-dr/dr_restore_all.py` with CLI `bkp-dr/restore-all.py`. It validates completed metadata, exact checksum coverage, stored file hashes, source-commit availability, manifest strategy/restore-phase correspondence and lifecycle ordering, then emits `global -> pre-prepare -> prepare -> post-prepare-pre-deploy -> deploy -> post-deploy -> external -> verify`. Host-qualify this planner against the canonical recovery point, then implement strategy-driven mutation against an isolated clean target. Real execution remains intentionally blocked until that qualification is complete.
...
The first generic inverse-path milestone is implemented: `dr_restore_all.py` plus `restore-all.py --dry-run` performs strict read-only full-set preflight and phase planning without writing `.env`, runtime data, databases, repositories or containers. Focused tests cover phase ordering, manifest strategy drift, mandatory global operational environment and exact checksum-index coverage. Real mutation is still blocked by design pending host qualification and isolated-target adapters.
