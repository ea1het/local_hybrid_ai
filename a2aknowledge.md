...
The generic inverse-path planner `bkp-dr/dr_restore_all.py` plus `bkp-dr/restore-all.py --dry-run` is host-qualified against the canonical recovery point. It validated five checksummed files, reproduced the phase order `global -> pre-prepare -> prepare -> post-prepare-pre-deploy -> deploy -> post-deploy -> external -> verify`, failed closed when real execution was attempted, left the installer converged and runtime unchanged, and ended with clean Git state.

The next bounded execution slice is `bkp-dr/dr_restore_stage.py` plus `bkp-dr/restore-stage.py`. It may execute only against an isolated empty destination: materialize the recorded Git commit, restore `operational.env` mode `0600`, verify `REQUIRE`/`external-config` prerequisites from that staged env, and restore `pre-prepare` archive artifacts into an isolated runtime tree. It must not run PREPARE/DEPLOY, restore PostgreSQL/Gitea into live services, mutate external Git or touch `/opt/docker/runtime`. On failure it may remove only its own isolated stage.

The next P0 is host qualification of that isolated staging slice, followed by isolated service-level PostgreSQL/Gitea restore and only then lifecycle execution in a clean target. The first end-to-end `restore all` qualification must not use the reference host as a destructive target. Exact evidence and continuation details are in [`bkp-dr/STATUS.md`](bkp-dr/STATUS.md).
