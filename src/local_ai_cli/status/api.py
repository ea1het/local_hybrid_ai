...
def _stack_name(directory):
    name = directory.split("_-_", 1)[1] if "_-_" in directory else directory
...
def _runtime_summary(entry, state):
    ...
    states = [state["containers"].get(n, "absent") for n in required]
    running = [install.is_running(v) for v in states]
    healthy = [install.is_runtime_healthy(v) for v in states]
    if all(running):
...
def stack_inventory():
    ...
    for sid in sorted(manifests):
        ...
        rows.append(
            stack_contracts.json_payload(
                sid, manifest["directory"], name=_stack_name(manifest["directory"]), state=state, health=health
            )
        )
    return rows
...
def json_payload():
    try:
        return {"schema_version": SCHEMA_VERSION, "command": "status", "success": True, "stacks": stack_inventory()}
    except (
        install.ManifestError,
        stack_contracts.StackContractError,
        StatusError,
        OSError,
...
def _human_stack_id(stack):
    return str(stack).removeprefix("stack")


def cli_text(payload):
    if not payload["success"]:
        return f"STATUS ERROR [STATUS_STATE_INVALID]: {payload['error']['message']}"
    headers = ("STACK", "NAME", "STATE", "HEALTH")
    values = [headers] + [(_human_stack_id(r["stack"]), r["name"], r["state"], r["health"]) for r in payload["stacks"]]
