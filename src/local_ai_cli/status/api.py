...
def cli_text(payload):
 ...
 headers=("STACK","NAME","STATE","HEALTH");values=[headers]+[(_human_stack_id(r["stack"]),r["name"],r["state"],r["health"]) for r in payload["stacks"]];widths=[max(len(str(row[i])) for row in values) for i in range(len(headers))];lines=[]
