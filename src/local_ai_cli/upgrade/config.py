...
class UpgradeError(RuntimeError):
    def __init__(self, message, *, code="UPGRADE_ERROR", recovery_point=None):
...
def registry_cache_ttl_seconds():
...
def _save_registry_cache(entries):
    ordered = sorted(entries.items(), key=lambda item: float(item[1].get("stored_at", 0)), reverse=True)[
        :REGISTRY_CACHE_MAX_ENTRIES
    ]
    upgrade_cache.save(registry_cache_path(), dict(ordered))
...
def _registry_cache_key(component, image, local_digest):
...
def _cached_registry_state(component, image):
    ...
    if not isinstance(stored_at, (int, float)) or time.time() - float(stored_at) > ttl or not isinstance(state, dict):
        return None
    try:
...
def load_catalog_raw():
    ...
    except component_inventory.ComponentInventoryError as exc:
...
def compose_image(component, env):
    return upgrade_runtime.compose_image(ROOT, component, env)


def running_image(component):
...
def _execution_metadata(record, component):
...
def _registry_availability(component, image, *, online):
    ...
    try:
        ...
        raise UpgradeError(
            f"registry discovery failed for {key(component)}: {exc}", code="UPGRADE_REGISTRY_SOURCE_INVALID"
        ) from exc
...
def _inventory_availability(component, record, observed_image, desired_image, *, query_upstream, evaluate):
    ...
    if availability in ("local", "n/a"):
        return availability, None, ("local" if availability == "local" and observed_image else actual)
    if not evaluate:
        return "not-evaluated", None, actual
    available, registry, discovered = _registry_availability(
        component, observed_image or desired_image, online=query_upstream
    )
    display = (
        upgrade_registry.display_label(observed_image, discovered_version=discovered) if observed_image else actual
    )
    return available, registry, display
...
def inventory(*, query_upstream=True):
    ...
    for component in load_catalog():
        ...
        observed = running_image(component)
        ...
        available, registry, actual_display = _inventory_availability(
            ...
            evaluate=effective_selectable,
        )
        actual = version_from_image(observed)
        ...
        rows.append(
            {
                ...
                "available": available,
                "drift": upgrade_inventory.drift_for(available),
                "policy": policy_state["effective_policy"],
                "selectable": effective_selectable,
...
def human_stack_id(stack):
    return upgrade_inventory.human_stack_id(stack)


def _human_available(row):
...
def find_component(stack, name):
    ...
    if not upgrade_policy.effective_selectable(runtime_root(), key(component), component.selectable)[2]:
        ...
        raise UpgradeError(
            f"component is inventory-only: {key(component)} ({blocked_by})", code="UPGRADE_COMPONENT_NOT_SELECTABLE"
        )
    return component
