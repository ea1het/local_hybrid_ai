...
def drift_for(available: str) -> str:
    if available == "current":
        return "no"
    if available in {"n/a", "unchecked", "local", "pinned", "unknown", "not-evaluated"}:
        return "n/a"
    return "yes"
...
def human_available(row: dict) -> str:
    ...
    if value == "unknown":
        ...
        status = registry.get("remote_status") or registry.get("tags_status")
        reasons = {"rate_limited": "rate limited", "unauthorized": "unauthorized", "forbidden": "forbidden"}
        if status in reasons:
            return f"unknown ({reasons[status]})"
    if value in {"n/a", "unchecked", "pinned", "unknown", "current", "update"}:
        return value
    return str(value)
