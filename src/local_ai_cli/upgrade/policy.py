...
def _save_overrides(runtime_root:Path,overrides:dict[str,str])->None:
 path=policy_path(runtime_root);path.parent.mkdir(parents=True,exist_ok=True);payload={"schema_version":SCHEMA_VERSION,"overrides":dict(sorted(overrides.items()))};tmp=path.with_suffix(".tmp");tmp.write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n",encoding="utf-8");os.replace(tmp,path)
def set_override(runtime_root:Path,component_key:str,policy:str)->None:
 ...
 overrides=load_overrides(runtime_root);overrides[component_key]=policy;_save_overrides(runtime_root,overrides)
def clear_override(runtime_root:Path,component_key:str)->None:
 overrides=load_overrides(runtime_root);overrides.pop(component_key,None);_save_overrides(runtime_root,overrides)
...
def selection_status(runtime_root:Path,component_key:str,component:dict,selection:dict|None)->dict:
 ...
 if selection:
  current=selection.get("current_at_selection");target=selection.get("version");accepted=component.get("selectable",True) or selection.get("forced") is True;result["selection_valid"]=bool(isinstance(current,str) and isinstance(target,str) and accepted and target_supported(effective,current,target))
 return result
