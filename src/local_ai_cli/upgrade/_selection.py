...
def apply_recipe_available(record):
    ...
    has_deploy = isinstance(apply.get("deploy"), list) and bool(apply.get("deploy"))
    if apply.get("type") == "env-version":
        return has_deploy and all(
            isinstance(apply.get(k), str) and bool(apply.get(k)) for k in ("env_key", "image_env_key")
        )
    if apply.get("type") == upgrade_postgres_migration.TYPE:
        return has_deploy and all(
            isinstance(apply.get(k), str) and bool(apply.get(k)) for k in upgrade_postgres_migration.REQUIRED_APPLY_KEYS
        )
    return False
...
def require_selection_permission(component):
    if effective_selectable(component)[2]:
        return
    record = component_record(component)
    blocked = (record.get("execution") or {}).get("blocked_by")
    raise upgrade.UpgradeError(
        f"component is not selectable: {upgrade.key(component)} ({blocked}); enable it with 'upgrade selectable {upgrade.human_stack_id(component.stack)} {component.name} enable --yes'",
...
def target_reference(component, version, env):
    ...
    if not image:
        raise upgrade.UpgradeError(
            f"cannot determine image repository for {upgrade.key(component)}", code="UPGRADE_TARGET_NOT_AVAILABLE"
        )
...
def validate_immutable_target(component, selection, env):
    target, stored_ref, stored_digest = (
        selection.get("version"),
        selection.get("target_image"),
        selection.get("target_digest"),
    )
    if not all(isinstance(v, str) and v for v in (target, stored_ref, stored_digest)):
        raise upgrade.UpgradeError(
...
def validate_selected_baselines(selections):
    ...
    for selection in selections:
        ...
        if component is None:
            raise upgrade.UpgradeError(
                f"selected component no longer exists: {component_key}", code="UPGRADE_PLAN_STALE"
            )
        if not effective_selectable(component)[2]:
            raise upgrade.UpgradeError(
                f"selected component is no longer selectable: {component_key}", code="UPGRADE_COMPONENT_NOT_SELECTABLE"
            )
        ...
        if target == current:
            raise upgrade.UpgradeError(
                f"upgrade target is already current for {component_key}: {current}", code="UPGRADE_PLAN_STALE"
            )
        try:
            effective = upgrade_policy.effective_policy(upgrade.runtime_root(), component_key, records[component_key])[
                2
            ]
        except upgrade_policy.PolicyError as exc:
            raise upgrade.UpgradeError(str(exc), code="UPGRADE_POLICY_INVALID") from exc
...
def execution_records_for(selections):
    ...
    for selection in selections:
        ...
        if component_key in result:
            ...
            _, _, effective = upgrade_policy.effective_selectable(
                upgrade.runtime_root(), component_key, record.get("selectable", True)
            )
            record["selectable"] = effective
