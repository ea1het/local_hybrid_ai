...
def _running_image(root, container):
    return upgrade_runtime.running_container_image(root, container)


def _version_from_image(image):
...
def _wait_ready(root, names, *, timeout_seconds=180):
    ...
    while True:
        ...
        if all(_healthy(v) for v in last.values()):
            return
        terminal = {
            k: v
            for k, v in last.items()
            if v in {"absent", "dead", "exited"} or v.startswith("dead/") or v.startswith("exited/")
        }
        if terminal:
            raise UpgradeExecutionError(
...
def _atomic_update_env(path, updates):
    ...
    try:
        ...
        try:
            ...
            pass
        raise UpgradeExecutionError(
            "UPGRADE_ENV_WRITE_FAILED", f"cannot update operational .env atomically: {exc}"
        ) from exc
...
def _target_image_ref(component_key, component, selection, env):
    ...
    if not isinstance(image_env_key, str) or not image_env_key:
        raise UpgradeExecutionError(
            "UPGRADE_INTERNAL_CONFIG", f"component has no image_env_key for target preflight: {component_key}"
        )
    repository = env.get(image_env_key)
    if not repository:
        raise UpgradeExecutionError(
            "UPGRADE_ENV_KEY_MISSING", f"target image repository key is missing from operational .env: {image_env_key}"
        )
    ...
    try:
        ...
        raise UpgradeExecutionError(
            "UPGRADE_INTERNAL_CONFIG", f"invalid target image repository for {component_key}: {exc}"
        ) from exc
...
def _verify_selected_digest(component_key, image_ref, selection):
    ...
    if not all(isinstance(v, str) and v for v in (selected_ref, selected_digest)):
        raise UpgradeExecutionError(
            "UPGRADE_PLAN_STALE", f"selected target has no immutable identity for {component_key}; reselect the target"
        )
    ...
    try:
        ...
        raise UpgradeExecutionError(
            "UPGRADE_TARGET_NOT_AVAILABLE", f"cannot resolve selected target {image_ref}: {exc}"
        ) from exc
    if probe.status != "ok" or not probe.digest:
        raise UpgradeExecutionError(
            "UPGRADE_TARGET_NOT_AVAILABLE", f"selected target is not available: {image_ref} ({probe.status})"
        )
...
def _preflight_target_image(root, image_ref):
    try:
        ...
        raise UpgradeExecutionError(
            "UPGRADE_TARGET_PREFLIGHT_FAILED", f"cannot inspect target image {image_ref}: {exc}"
        ) from exc
    if cp.returncode != 0:
...
def _recovery_point():
    ...
    if not payload.get("success"):
        ...
        raise UpgradeExecutionError(
            "UPGRADE_BACKUP_FAILED", f"backup engine failed: {error.get('message','backup operation failed')}"
        )
    result = payload.get("result")
    path = result.get("backup_set") if isinstance(result, dict) else None
...
def execute(
    *,
    ...
    plan_path: Path,
    quiet: bool = False,
    confirm_data_migration: bool = False,
) -> dict:
    if not selections:
    ...
    lifecycle = _load_json(root / "src" / "local_ai_cli" / "install-lifecycle.json")
    ...
    for selection in selections:
        ...
        if component is None:
            raise UpgradeExecutionError(
                "UPGRADE_COMPONENT_UNKNOWN", f"selected component is not in catalog: {component_key}"
            )
        if not component.get("selectable", True):
            raise UpgradeExecutionError(
                "UPGRADE_COMPONENT_NOT_SELECTABLE", f"selected component is not executable by policy: {component_key}"
            )
        try:
        ...
        apply = component.get("apply")
        apply_type = apply.get("type") if isinstance(apply, dict) else None
        if apply_type == "env-version":
            env_key = apply.get("env_key")
            if not isinstance(env_key, str) or not env_key:
                raise UpgradeExecutionError(
                    "UPGRADE_INTERNAL_CONFIG", f"component has invalid env_key: {component_key}"
                )
            env_updates[env_key] = selection["version"]
        elif apply_type == upgrade_postgres_migration.TYPE:
            if len(selections) != 1:
                raise UpgradeExecutionError(
                    "UPGRADE_DATA_MIGRATION_MUST_BE_ISOLATED",
                    f"{component_key} performs a data migration and must be applied alone, not combined with other selections",
                )
            if not confirm_data_migration:
                raise UpgradeExecutionError(
                    "UPGRADE_DATA_MIGRATION_CONFIRMATION_REQUIRED", f"{component_key} requires --confirm-data-migration"
                )
        else:
            raise UpgradeExecutionError(
                "UPGRADE_COMPONENT_NOT_EXECUTABLE", f"component has no safe executor: {component_key}"
            )
        ...
        for sid in sorted(affected_stacks):
            ...
            for selection in selected_for_stack:
                ...
                if apply.get("type") == upgrade_postgres_migration.TYPE:
                    upgrade_postgres_migration.execute(
                        root=root,
                        runtime_root=runtime_root,
                        stack_directory=entry["directory"],
                        component_key=f"{selection['stack']}/{selection['component']}",
                        apply=apply,
                        container=component["container"],
                        target_image_ref=selection["target_image"],
                        env_path=env_path,
        ...
        for provider_sid in sorted(affected_stacks):
            ...
            for sid, manifest in manifests.items():
                if sid in affected_stacks or not _prepared(root, manifest):
                    continue
                depends = provider_sid in set(manifest.get("requires", [])) | set(manifest.get("optional", []))
                consumes = bool(
                    provider_caps & (set(manifest.get("consumes", [])) | set(manifest.get("optional_consumes", [])))
                )
                if not (depends or consumes):
                    continue
                entry = lifecycle["stacks"][str(sid)]
                _run_commands(root, entry["directory"], entry.get("verify", []), quiet=quiet)
