...
# License, v. 2.0.
"""Public upgrade command API: argparse surface, payload construction and CLI-text rendering. Global CLI options belong to ./local-ai."""

from __future__ import annotations
import argparse, json
from . import config as upgrade
from . import engine as upgrade_executor
from . import _policy as upgrade_policy
from . import _selection as upgrade_selection


def public_parser():
    p = argparse.ArgumentParser(
        prog="local-ai upgrade",
        description="Inspect versions, stage component upgrades, manage policy and apply a staged plan",
    )
    p.add_argument(
        "--offline", action="store_true", help="inspect installed state without querying upstream registries"
    )
    p.add_argument(
        "--json", action="store_true", help="emit one machine-readable JSON document; global option owned by local-ai"
    )
    p.add_argument(
        "--yes",
        action="store_true",
        help="grant non-interactive consent for mutations; global option owned by local-ai",
    )
    p.add_argument(
        "--confirm-data-migration",
        action="store_true",
        help="required in addition to --yes to apply a staged selection that performs a data migration (e.g. a PostgreSQL major-version upgrade)",
    )
    p.add_argument(
        "arguments",
        nargs="*",
        metavar="ARG",
        help="check | STACK [COMPONENT] <select VERSION|clear> | policy STACK [COMPONENT] [set POLICY|clear] | selectable STACK [COMPONENT] [enable|disable|clear] | adopt",
    )
    return p
...
def _require_selection_permission(component):
    return upgrade_selection.require_selection_permission(component)


def select_payload(stack, component_name, version):
    component = _resolve_component(stack, component_name)
    _require_selection_permission(component)
...
def selected_records():
    return [dict(v) for _, v in sorted(upgrade.load_plan()["selected"].items())]
...
def execute_payload(*, confirm_data_migration=False):
    ...
    try:
        result = upgrade_executor.execute(
            ...
            quiet=True,
            confirm_data_migration=confirm_data_migration,
        )
...
def _policy_record(stack, component_name):
    ...
    matches = [(k, v) for k, v in records.items() if v.get("stack") == stack]
    if not matches:
...
def _policy_row(key, record, plan):
    try:
        state = upgrade_policy.selection_status(upgrade.runtime_root(), key, record, plan["selected"].get(key))
    except upgrade_policy.PolicyError as exc:
        raise upgrade.UpgradeError(str(exc), code="UPGRADE_POLICY_INVALID") from exc
    effective = upgrade_policy.effective_selectable(upgrade.runtime_root(), key, record.get("selectable", True))[2]
    return {
        "stack": record["stack"],
        "component": record["id"],
        **state,
        "selectable": effective,
        "selected": plan["selected"].get(key, {}).get("version"),
    }


def _selectable_row(key, record):
    default, override, effective = upgrade_policy.effective_selectable(
        upgrade.runtime_root(), key, record.get("selectable", True)
    )
    return {
        "stack": record["stack"],
        "component": record["id"],
        "default_selectable": default,
        "override_selectable": override,
        "effective_selectable": effective,
        "executable": upgrade_selection.apply_recipe_available(record),
    }
...
def selectable_payload(args):
    ...
    if not args:
        return {
            "schema_version": upgrade.SCHEMA_VERSION,
            "command": "upgrade.selectable",
            "success": True,
            "components": [_selectable_row(k, r) for k, r in sorted(records.items())],
        }
    action = None
    left = list(args)
    if left[-1:] == ["enable"]:
        action, left = "enable", left[:-1]
    elif left[-1:] == ["disable"]:
        action, left = "disable", left[:-1]
    elif left[-1:] == ["clear"]:
        action, left = "clear", left[:-1]
    if len(left) not in (1, 2):
        raise upgrade.UpgradeError("invalid selectable syntax", code="UPGRADE_USAGE")
    key, record = _policy_record(left[0], left[1] if len(left) == 2 else None)
    before = _selectable_row(key, record)
    if action == "enable":
        upgrade_policy.set_selectable_override(upgrade.runtime_root(), key, True)
    elif action == "disable":
        upgrade_policy.set_selectable_override(upgrade.runtime_root(), key, False)
    elif action == "clear":
        upgrade_policy.clear_selectable_override(upgrade.runtime_root(), key)
    after = _selectable_row(key, record)
    payload = {
        "schema_version": upgrade.SCHEMA_VERSION,
        "command": "upgrade.selectable",
        "success": True,
    ...
    if action:
        payload["previous_effective_selectable"] = before["effective_selectable"]
    return payload
...
def policy_payload(args):
    ...
    if not args:
        return {
            ...
            "success": True,
            "components": [_policy_row(k, r, plan) for k, r in sorted(records.items())],
        }
    action = policy = None
    left = list(args)
    if "set" in left:
    ...
    key, record = _policy_record(left[0], left[1] if len(left) == 2 else None)
    before = _policy_row(key, record, plan)
    if action == "set":
        ...
        upgrade_policy.clear_override(upgrade.runtime_root(), key)
    after = _policy_row(key, record, plan)
    payload = {
        "schema_version": upgrade.SCHEMA_VERSION,
        "command": "upgrade.policy",
        "success": True,
...
def cli_text(payload):
    ...
    if not payload.get("success"):
        return f"UPGRADE ERROR [{payload['error']['code']}]: {payload['error']['message']}"
    if command == "upgrade.check":
        rows = payload.get("components", [])
        headers = ("STACK", "COMPONENT", "CURRENT", "AVAILABLE", "POLICY", "DRIFT")
        values = [headers]
        for row in rows:
            values.append(
                (
                    upgrade.human_stack_id(row.get("stack", "")),
                    row.get("component", "-"),
                    row.get("current") or "-",
                    row.get("available") or "-",
                    row.get("effective_policy") or row.get("policy") or "-",
                    row.get("drift") or "-",
                )
            )
        widths = [max(len(str(r[i])) for r in values) for i in range(len(headers))]
        lines = []
        for idx, row in enumerate(values):
            lines.append("  ".join(str(v).ljust(widths[i]) for i, v in enumerate(row)))
            lines.extend(["  ".join("-" * w for w in widths)] if idx == 0 else [])
        return "\n".join(lines)
    if command == "upgrade.select":
        s = payload["selection"]
        return f"Selected {s['stack']}/{s['component']}: {s['current_at_selection']} -> {s['version']} ({s['policy_at_selection']}, {s['target_digest']})"
    if command == "upgrade.clear":
        return f"Cleared {payload['stack']}/{payload['component']}"
    if command == "upgrade.apply":
        lines = ["UPGRADE: PASS"]
        if payload.get("recovery_point"):
            lines.append(f"- recovery point: {payload['recovery_point']}")
        for item in payload["upgraded"]:
            lines.append(f"- {item['stack']}/{item['component']}: {item['current_at_selection']} -> {item['version']}")
        if payload["reverified_stacks"]:
            lines.append("- reverified consumers: " + ", ".join(payload["reverified_stacks"]))
        return "\n".join(lines)
    if command == "upgrade.policy":
        rows = payload.get("components") or [payload]
    ...
    if command == "upgrade.selectable":
        rows = payload.get("components") or [payload]
        headers = ("STACK", "COMPONENT", "DEFAULT", "OVERRIDE", "EFFECTIVE", "EXECUTABLE")
        values = [headers]
        for row in rows:
            values.append(
                (
                    upgrade.human_stack_id(row["stack"]),
                    row["component"],
                    "yes" if row["default_selectable"] else "no",
                    "-" if row["override_selectable"] is None else ("yes" if row["override_selectable"] else "no"),
                    "yes" if row["effective_selectable"] else "no",
...
def build_payload(args, *, apply_selected=False, confirm_data_migration=False):
    try:
        ...
        if not args or args == ["check"]:
            return json_payload(upgrade.inventory(query_upstream=True)), 0
        if args in (["--offline"], ["check", "--offline"], ["--offline", "check"]):
            return json_payload(upgrade.inventory(query_upstream=False)), 0
        if args and args[0] == "policy":
            return policy_payload(args[1:]), 0
        if args and args[0] == "selectable":
            return selectable_payload(args[1:]), 0
        if "select" in args:
        ...
        if args[-1:] == ["clear"] and len(args) in (2, 3):
            left = args[:-1]
            return clear_payload(left[0], left[1] if len(left) == 2 else None), 0
        raise upgrade.UpgradeError("invalid upgrade syntax", code="UPGRADE_USAGE")
