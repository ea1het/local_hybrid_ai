...
def execute(
    *, root: Path, runtime_root: Path, stack_directory: str, component_key: str,
    ...
    if missing or not isinstance(deploy, list) or not deploy:
        raise UpgradeExecutionError("UPGRADE_INTERNAL_CONFIG", f"component has an incomplete postgres-major-upgrade recipe: {component_key}")
    ...
    if not isinstance(dependents, list) or not all(isinstance(d, str) and d for d in dependents):
        raise UpgradeExecutionError("UPGRADE_INTERNAL_CONFIG", f"component has an invalid dependent_containers list: {component_key}")

    image_env_key, database_env, role_env, data_path_env, data_relative_path = (apply[key] for key in REQUIRED_APPLY_KEYS)
    ...
    if not database or not role or not base_path:
        raise UpgradeExecutionError("UPGRADE_ENV_KEY_MISSING", f"postgres-major-upgrade requires {database_env}, {role_env} and {data_path_env} in the operational .env")
    try:
    ...
    if _container_state(container) not in {"running", "running/healthy"}:
        raise UpgradeExecutionError("UPGRADE_TARGET_NOT_RUNNING", f"{container} must be running before a major-version migration")
    ...
    cp = pg.run_binary_to_file(
        pg.docker_admin_prefix() + ["pg_dump", "-h", "127.0.0.1", "-U", pg.ADMIN_USER, "-d", database, "--format=custom", "--no-owner", "--no-acl"],
        dump_path,
    )
    if cp.returncode != 0:
        pg.fail_command("pg_dump", cp)
    if dump_path.stat().st_size <= 0:
        raise UpgradeExecutionError("UPGRADE_BACKUP_INVALID", "postgres-major-upgrade dump is empty; aborting before any mutation")
    ...
    try:
        ...
        raise UpgradeExecutionError("UPGRADE_COMMAND_FAILED", f"cannot move aside the old postgres data directory for {component_key}: {exc}; dump preserved at {dump_path}") from exc

    _atomic_update_env(env_path, {image_env_key: target_image_ref})

    compose_file = root / stack_directory / "docker-compose.yml"
    cp = _run(["docker", "compose", "--env-file", str(env_path), "-f", str(compose_file), "up", "-d", container], cwd=root / stack_directory)
    if cp.returncode != 0:
        raise UpgradeExecutionError("UPGRADE_COMMAND_FAILED", f"cannot start {container} on the new major version for {component_key}; old data directory preserved at {aside}, dump preserved at {dump_path}: {(cp.stderr or '').strip()}")
    ...
    if cp.returncode != 0:
        ...
        raise UpgradeExecutionError("UPGRADE_COMMAND_FAILED", f"postgres-major-upgrade deploy step failed for {component_key}; old data directory preserved at {aside}, dump preserved at {dump_path}" + (f": {detail}" if detail else ""))
    ...
    if cp.returncode != 0:
        ...
        raise UpgradeExecutionError("UPGRADE_COMMAND_FAILED", f"pg_restore failed for {component_key}; old data directory preserved at {aside}, dump preserved at {dump_path}: {detail}")

    restored_tables = pg.list_user_tables(database)
    if restored_tables != source_tables:
        raise UpgradeExecutionError("UPGRADE_TARGET_NOT_RUNNING", f"restored table inventory does not match the pre-upgrade source for {component_key}; old data directory preserved at {aside}, dump preserved at {dump_path}")

    for dependent in dependents:
        cp = _run(["docker", "start", dependent])
        if cp.returncode != 0:
            raise UpgradeExecutionError("UPGRADE_COMMAND_FAILED", f"postgres-major-upgrade succeeded for {component_key} but {dependent} did not restart; start it manually: {(cp.stderr or '').strip()}")
