...
AUTHORITIES = {
    ...
    "stack2/rabbitmq": {
        "type": "split",
        "image_key": "FIRECRAWL_RABBITMQ_IMAGE",
        "version_key": "FIRECRAWL_RABBITMQ_VERSION",
    },
...
class AdoptionError(RuntimeError):
    def __init__(self, message, *, code="UPGRADE_ADOPTION_ERROR"):
...
def _repository_text(reference):
    ...
    if reference.registry == "docker.io" and repository.startswith("library/"):
...
def _component_prepared(component):
    return True if not component.compose else (upgrade.ROOT / component.compose).parent.joinpath(".lock").is_file()
...
def desired_updates():
    ...
    for component_key, authority in AUTHORITIES.items():
        ...
        if authority["type"] == "split":
            ...
            adopted.append(
                {"component": component_key, "image": repository, "version": version, "running_image": running}
            )
        else:
...
def _validate_conflicts(current, expected):
    ...
    if conflicts:
        raise AdoptionError(
            "operational version authority conflicts with observed runtime: " + ", ".join(conflicts),
...
def _apply_missing(path, expected):
    ...
    _validate_conflicts(current, expected)
    ...
    try:
        ...
        try:
            ...
            pass
        raise AdoptionError(
            f"cannot update operational .env atomically: {exc}", code="UPGRADE_ADOPTION_ENV_WRITE_FAILED"
        ) from exc
...
def cli_text(payload):
    ...
    for item in payload["components"]:
        lines.append(
            f"- {item['component']}: skipped ({item['skipped']})"
            if "skipped" in item
            else f"- {item['component']}: {item['version']}"
        )
    if payload["missing_keys"]:
        lines.append("- missing operational keys: " + ", ".join(payload["missing_keys"]))
    else:
        lines.append("- operational version authority already matches observed installed stacks")
    if not payload["executed"] and payload["missing_keys"]:
        lines.append("- no changes made; run `./local-ai upgrade adopt --yes` to persist this exact runtime baseline")
    return "\n".join(lines)
