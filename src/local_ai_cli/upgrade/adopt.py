...
def _read_operational_env(path):
 ...
 except OSError as exc:raise AdoptionError(f"cannot read operational .env: {exc}",code="UPGRADE_ADOPTION_ENV_READ_FAILED") from exc
 values={}
 for raw in lines:
  line=raw.strip()
...
def _split_identity(component,running):
 ...
 if _tracking_tag(version):
  ...
  except upgrade_registry.RegistryError as exc:raise AdoptionError(f"cannot resolve installed version for {upgrade.key(component)}: {exc}",code="UPGRADE_ADOPTION_IDENTITY_UNRESOLVED") from exc
  if state is not None and state.current_version:version=state.current_version
 if not version:raise AdoptionError(f"cannot resolve installed version for {upgrade.key(component)}",code="UPGRADE_ADOPTION_IDENTITY_UNRESOLVED")
 return _repository_text(reference),version
def _component_prepared(component):return True if not component.compose else (upgrade.ROOT/component.compose).parent.joinpath(".lock").is_file()
def desired_updates():
 components={upgrade.key(component):component for component in upgrade.load_catalog()};updates={};adopted=[]
 for component_key,authority in AUTHORITIES.items():
  component=components.get(component_key)
  if component is None:raise AdoptionError(f"version authority references unknown component: {component_key}",code="UPGRADE_ADOPTION_CONFIG_INVALID")
  running=upgrade.running_image(component)
  if not running:
   if not _component_prepared(component):adopted.append({"component":component_key,"skipped":"stack-not-prepared"});continue
   raise AdoptionError(f"cannot adopt {component_key}: PREPARED component image is unavailable",code="UPGRADE_ADOPTION_RUNTIME_UNAVAILABLE")
  if authority["type"]=="split":
   repository,version=_split_identity(component,running);updates[authority["image_key"]]=repository;updates[authority["version_key"]]=version;adopted.append({"component":component_key,"image":repository,"version":version,"running_image":running})
  else:
...
def cli_text(payload):
 ...
 for item in payload["components"]:lines.append(f"- {item['component']}: skipped ({item['skipped']})" if "skipped" in item else f"- {item['component']}: {item['version']}")
 if payload["missing_keys"]:lines.append("- missing operational keys: "+", ".join(payload["missing_keys"]))
