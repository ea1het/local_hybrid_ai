...
def _load_state(runtime_root: Path) -> dict:
    ...
    if not path.is_file():
        return {"schema_version": SCHEMA_VERSION, "overrides": {}, "selectable_overrides": {}}
    try:
    ...
    if not isinstance(payload.get("selectable_overrides", {}), dict):
        raise PolicyError("unsupported upgrade policy state schema")
    return payload
...
def _save_state(runtime_root: Path, state: dict) -> None:
    ...
    payload = {
        ...
        "overrides": dict(sorted(state.get("overrides", {}).items())),
        "selectable_overrides": dict(sorted(state.get("selectable_overrides", {}).items())),
    }
...
def set_override(runtime_root: Path, component_key: str, policy: str) -> None:
    ...
    state["overrides"][component_key] = policy
    _save_state(runtime_root, state)


def clear_override(runtime_root: Path, component_key: str) -> None:
    state = _load_state(runtime_root)
    state["overrides"].pop(component_key, None)
    _save_state(runtime_root, state)
...
def load_selectable_overrides(runtime_root: Path) -> dict[str, bool]:
    ...
    for component_key, value in _load_state(runtime_root).get("selectable_overrides", {}).items():
        if not isinstance(value, bool):
            raise PolicyError(f"invalid selectable override for {component_key}: {value}")
...
def set_selectable_override(runtime_root: Path, component_key: str, selectable: bool) -> None:
    state = _load_state(runtime_root)
    state["selectable_overrides"][component_key] = bool(selectable)
    _save_state(runtime_root, state)


def clear_selectable_override(runtime_root: Path, component_key: str) -> None:
    state = _load_state(runtime_root)
    state["selectable_overrides"].pop(component_key, None)
    _save_state(runtime_root, state)


def effective_selectable(
    runtime_root: Path, component_key: str, manifest_default: bool
) -> tuple[bool, bool | None, bool]:
    override = load_selectable_overrides(runtime_root).get(component_key)
    return bool(manifest_default), override, override if override is not None else bool(manifest_default)
...
def target_supported(policy: str, current: str, target: str) -> bool:
    ...
    b = version_tuple(target)
    if policy == "manual":
        return b > a if a is not None and b is not None else True
...
def selection_status(runtime_root: Path, component_key: str, component: dict, selection: dict | None) -> dict:
    ...
    if selection:
        ...
        accepted = effective_selectable(runtime_root, component_key, component.get("selectable", True))[2]
