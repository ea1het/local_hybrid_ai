...
def resolve_identity(component, desired_image, actual_image):
    ...
    if desired_floating:
        if not actual_image or not getattr(component, "container", None):
            return desired, actual, "n/a"
        try:
            state = upgrade_registry.inspect(component.container, actual_image, tracking_image=desired_image)
        ...
        if state.local_digest and state.remote_digest:
            resolved = "no" if state.local_digest == state.remote_digest else "yes"
        elif state.current_version and state.available_version:
            resolved = "no" if state.current_version == state.available_version else "yes"
        else:
            resolved = "n/a"
...
def deployed_versions(runtime_root: Path):
    ...
    for number, line in enumerate(lines, start=1):
        ...
        for item in upgraded:
            ...
            if all(isinstance(v, str) and v for v in (stack, component, version)):
