...
class RuntimeResult:
    ...
    def as_dict(self):
        return stack_contracts.json_payload(
            self.stack_id,
            self.directory,
...
def _required_containers(sid, lifecycle):
    return tuple(lifecycle["stacks"][str(sid)]["required_containers"])


def _owned_containers(manifest):
    return tuple(
        v.split(":", 1)[1] for v in manifest.get("owns", []) if isinstance(v, str) and v.startswith("container:")
    )


def _runtime_running(sid, lifecycle):
    required = _required_containers(sid, lifecycle)
    return bool(required) and all(install.is_running(install.container_state(n)) for n in required)


def _required_provider_ids(manifest):
    return {int(v) for v in manifest.get("requires", [])}


def _required_consumers(provider_sid, manifests, lifecycle):
    caps = set(manifests[provider_sid].get("provides", []))
    out = []
    for sid, manifest in manifests.items():
        if sid == provider_sid or not _runtime_running(sid, lifecycle):
            continue
        if provider_sid in _required_provider_ids(manifest) or bool(caps & set(manifest.get("consumes", []))):
            out.append(sid)
    return sorted(out)


def _missing_required_providers(sid, manifests, lifecycle):
    return [
        p
        for p in sorted(_required_provider_ids(manifests[sid]))
        if _required_containers(p, lifecycle) and not _runtime_running(p, lifecycle)
    ]
...
def _resolve_one(selector, manifests):
    ...
    except install.ManifestError as exc:
...
def execute(action, selector):
    ...
    if not required:
        raise RuntimeLifecycleError("STACK_RUNTIME_EMPTY", f"stack{sid} has no managed runtime containers to {action}")
    ...
    if action == "stop":
        ...
        if consumers:
            raise RuntimeLifecycleError(
                ...
                f"cannot stop stack{sid}; required consumers are running: {', '.join(f'stack{x}' for x in consumers)}",
            )
    else:
        ...
        if missing:
            raise RuntimeLifecycleError(
                ...
                f"cannot start stack{sid}; required providers are not running: {', '.join(f'stack{x}' for x in missing)}",
            )
...
def json_payload(action, selector):
    try:
        return execute(action, selector).as_dict()
...
def cli_text(payload):
    if not payload["success"]:
        return f"ERROR [{payload['error']['code']}]: {payload['error']['message']}"
...
def main(action, selector, *, json_output=False):
    ...
    payload = json_payload(action, selector)
    render.render_json(payload) if json_output else render.render_cli(cli_text(payload))
