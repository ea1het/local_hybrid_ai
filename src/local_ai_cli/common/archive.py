    ...
    while pending:
        ...
        for entry in entries:
            ...
            if stat.S_ISDIR(kind):
                ...
                raise ArchiveBackupError(f"archive source contains unsupported special file: {entry.path}")
    if not found:
...
def create_tar_archive(source: Path, destination: Path) -> None:
    ...
    try:
        with tarfile.open(destination, mode="x", format=tarfile.PAX_FORMAT, dereference=False) as archive:
...
def _validate_artifact_fields(item: dict, schema: dict, index: int, where: str, *, stack_owned: bool) -> None:
    ...
    require_exact_keys(item, required, f"{where} {index}")
    if stack_owned and (type(item["stack_id"]) is not int or item["stack_id"] < 0):
        raise ArchiveBackupError(f"{where} {index} stack_id is invalid")
    if not re.fullmatch(schema["properties"]["resource_id"]["pattern"], item["resource_id"]):
        raise ArchiveBackupError(f"{where} {index} resource_id is invalid")
    if item["strategy"] not in schema["properties"]["strategy"]["enum"]:
        raise ArchiveBackupError(f"{where} {index} strategy is invalid")
    if not isinstance(item["sensitive"], bool):
        raise ArchiveBackupError(f"{where} {index} sensitive is invalid")
    if item["restore_phase"] not in schema["properties"]["restore_phase"]["enum"]:
        raise ArchiveBackupError(f"{where} {index} restore_phase is invalid")
    if not re.fullmatch(schema["properties"]["relative_path"]["pattern"], item["relative_path"]):
        raise ArchiveBackupError(f"{where} {index} relative_path is invalid")
    if not re.fullmatch(schema["properties"]["sha256"]["pattern"], item["sha256"]):
        raise ArchiveBackupError(f"{where} {index} sha256 is invalid")
    if type(item["size_bytes"]) is not int or item["size_bytes"] < 0:
        raise ArchiveBackupError(f"{where} {index} size_bytes is invalid")
...
def validate_completed_metadata(metadata: dict, schema_path: Path = BACKUP_SCHEMA) -> None:
    ...
    if (
        not isinstance(metadata["requested"], list)
        or not metadata["requested"]
        or not all(isinstance(v, str) and v for v in metadata["requested"])
    ):
    ...
    if "global_artifacts" in metadata:
        if not isinstance(metadata["global_artifacts"], list):
            raise ArchiveBackupError("backup metadata global_artifacts must be an array")
        global_schema = schema["$defs"]["globalArtifact"]
        for index, artifact in enumerate(metadata["global_artifacts"]):
            if not isinstance(artifact, dict):
                raise ArchiveBackupError(f"global artifact {index} must be an object")
...
def execute_archive_backup_set(
    *,
    ...
    try:
        ...
        write_private(metadata_path, (json.dumps(metadata, indent=2, sort_keys=True) + "\n").encode())
        metadata_hash = sha256_file(metadata_path)
        checksums_path = temp / "checksums.sha256"
        write_private(
            checksums_path, (f"{artifact_hash}  {artifact.relative_path}\n{metadata_hash}  backup.json\n").encode()
        )
