...
def load_lifecycle():
    try:
        ...
        raise ManifestError(f"cannot read lifecycle registry: {exc}") from exc
    if data.get("schema_version") != 1 or not isinstance(data.get("stacks"), dict):
        raise ManifestError("unsupported src/local_ai_cli/install-lifecycle.json schema")
    return data
...
def validate_registry(manifests, lifecycle):
    ...
    if set(entries) != expected:
        raise ManifestError("lifecycle registry stack ids do not exactly match manifests")
    for sid, manifest in manifests.items():
        entry = entries[str(sid)]
        if entry.get("directory") != manifest["directory"]:
            raise ManifestError(f"stack{sid}: lifecycle directory disagrees with manifest")
        owned = {
        ...
        if not isinstance(required, list) or not all(isinstance(n, str) and n for n in required):
            raise ManifestError(f"stack{sid}: invalid required_containers registry")
        if not set(required).issubset(owned):
            raise ManifestError(f"stack{sid}: required_containers must be owned by the stack")


def stack_prepared(directory):
...
def resolve_requested(selectors, manifests):
    ...
    for token in selectors:
        sid = (
            int(token)
            if token.isdigit()
            else int(token[5:]) if token.startswith("stack") and token[5:].isdigit() else by_directory.get(token, -1)
        )
        if sid not in manifests:
            raise ManifestError(f"unknown stack selector: {token}")
        if sid not in out:
            out.append(sid)
    return out
...
def wait_required_runtime(sid, entry, *, timeout_seconds=180):
    ...
    while True:
        ...
        terminal = {
            ...
            if state in {"exited", "dead", "removing", "absent"} or state.startswith(("exited/", "dead/"))
        }
        if terminal:
            raise ManifestError(
                f"stack{sid} required runtime failed before READY: "
                + ", ".join(f"{n}={s}" for n, s in terminal.items())
            )
        if time.monotonic() >= deadline:
            raise ManifestError(
                f"stack{sid} required runtime did not become READY within {timeout_seconds}s: "
