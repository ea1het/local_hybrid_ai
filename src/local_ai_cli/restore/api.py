...
def list_backup_sets_payload(backup_root: str | Path | None = None) -> dict[str, object]:
    ...
    try:
        ...
        return {
            "schema_version": SCHEMA_VERSION,
            "command": "restore.list-backup-sets",
            "success": False,
            "error": {"code": "BACKUP_ROOT_INVALID", "message": str(exc)},
        }
    return {
        "schema_version": SCHEMA_VERSION,
        "command": "restore.list-backup-sets",
...
def plan_payload(backup_set: str | Path) -> dict[str, object]:
    return _operation(
        "restore.plan",
        "RESTORE_PLAN_FAILED",
        lambda: _load("_restore_all").plan_restore_all(Path(backup_set)).as_dict(),
    )
...
def drill_payload(backup_set: str | Path, destination: str | Path) -> dict[str, object]:
    return _operation(
        ...
        lambda: _load("_restore_drill").run_restore_drill(Path(backup_set), Path(destination)).as_dict(),
    )
...
def check_clean_target_payload(backup_set: str | Path) -> dict[str, object]:
    return _operation(
        ...
        lambda: _load("_restore_live_service").check_clean(Path(backup_set)),
    )
...
def resume_payload(backup_set: str | Path, memory_sync_ssh_bootstrap: str | Path) -> dict[str, object]:
    return _operation(
        "restore.resume",
        "RESTORE_RESUME_FAILED",
        lambda: _load("_restore_resume").resume(Path(backup_set), Path(memory_sync_ssh_bootstrap)),
    )
...
def cli_text(payload: dict[str, object]) -> str:
    if not payload.get("success"):
        ...
        if isinstance(error, dict):
            return f"RECOVERY ERROR [{error.get('code','RECOVERY_FAILED')}]: {error.get('message','recovery operation failed')}"
    ...
    if command == "restore.list-backup-sets":
        ...
        lines += [
            "STATUS     BACKUP SET                   SOURCE COMMIT  STACKS",
            "---------- ---------------------------- ------------- ------",
        ]
        for record in records:
            ...
            lines.append(
                f"{str(record.get('status','invalid')).upper():<10} {str(record.get('name','-')):<28} {source:<13} {stacks}"
            )
