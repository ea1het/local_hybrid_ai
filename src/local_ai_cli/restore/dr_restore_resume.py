...
import dr,dr_restore_all,dr_restore_compat,dr_restore_live,dr_stack4_restore_verify
...
def read_env_artifact(backup_set):
 metadata=dr_restore_all.read_completed_backup_set(backup_set);matches=[a for a in metadata.get("global_artifacts",[]) if a.get("resource_id")=="operational-env"]
...
def _recorded_installer_path(source_commit):
 for candidate in ("commands/install.py","installer/install.py","install.py"):
  if subprocess.run(["git","cat-file","-e",f"{source_commit}:{candidate}"],cwd=dr_restore_all.PROJECT_ROOT,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,check=False).returncode==0:return candidate
...
def verify_source(backup_set,stacks_root,source_commit):
 ...
 cp=subprocess.run(["git","show",f"{source_commit}:{relative}"],cwd=dr_restore_all.PROJECT_ROOT,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=False)
...
def postgres_table_count(values):
 db=dr.require_env_value(values,"LITELLM_DB_NAME",label="LITELLM_DB_NAME");text=require_ok(run(["docker","exec","litellm-postgres","psql","-At","-U","postgres","-d",db,"-c","SELECT count(*) FROM pg_tables WHERE schemaname NOT IN ('pg_catalog','information_schema');"]),"LiteLLM PostgreSQL verification")
 try:count=int(text)
...
def gitea_state(base_path):
 ...
 repos=dr_stack4_restore_verify.find_bare_repositories(base_path/"service_-_gitea"/"data"/"git"/"repositories")
 for repo in repos:dr_stack4_restore_verify.verify_repository(repo)
...
def verify_memory(base_path,values):
 service=dr.require_env_value(values,"HERMES_MEMORY_SERVICE",label="HERMES_MEMORY_SERVICE");branch=dr.require_env_value(values,"GITMEM_BRANCH",label="GITMEM_BRANCH");repo=base_path/service/"data";prefix=["git","-c",f"safe.directory={repo}","-C",str(repo)]
...
def verify_bootstrap(source,base_path,values):
 target=base_path/dr.require_env_value(values,"MEMORY_SYNC_SERVICE",label="MEMORY_SYNC_SERVICE")/"ssh"
...
def persist_git_memory_intent(base_path,values):
 ...
 try:uid=int(dr.require_env_value(values,"HERMES_UID",label="HERMES_UID"));gid=int(dr.require_env_value(values,"HERMES_GID",label="HERMES_GID"))
...
def enable_memory_sync(stacks_root,base_path,values):
 persist_git_memory_intent(base_path,values);container=dr.require_env_value(values,"MEMORY_SYNC_CONTAINER",label="MEMORY_SYNC_CONTAINER");cp=run(["docker","compose","--profile","git-memory","up","-d","--build","hermes-memory-sync"],cwd=stacks_root/"stack6_-_hermes");require_ok(cp,"Stack6 memory-sync profile restore");dr_restore_compat.wait_required_runtime(stacks_root,[6],timeout=240)
...
def _prepare_resume_context(backup_set,bootstrap):
 plan=dr_restore_all.plan_restore_all(backup_set);env_artifact,values=read_env_artifact(backup_set);return ResumeContext(backup_set,bootstrap,plan.source_commit,tuple(plan.resolved_stacks),env_artifact,values,dr_restore_live._absolute_safe_path(values,"STACKS_ROOT"),dr_restore_live._absolute_safe_path(values,"BASE_PATH"))
def resume(backup_set:Path,bootstrap:Path)->dict[str,object]:
 c=_prepare_resume_context(backup_set,bootstrap);verify_source(c.backup_set,c.stacks_root,c.source_commit);verify_env(c.env_artifact,c.stacks_root);verify_prepared(c.stacks_root,c.resolved_stacks);pg=postgres_table_count(c.values);gt,gr=gitea_state(c.base_path);mh=verify_memory(c.base_path,c.values);verify_bootstrap(c.bootstrap,c.base_path,c.values);resolved=list(c.resolved_stacks);dr_restore_compat.wait_required_runtime(c.stacks_root,resolved,timeout=240);dr_restore_compat.install_with_readiness_compat(c.stacks_root,resolved,reconcile=True,label="final restore READY/VERIFY/reconcile");enable_memory_sync(c.stacks_root,c.base_path,c.values);dr_restore_compat.install_with_readiness_compat(c.stacks_root,resolved,reconcile=False,label="post-resume final verification");return {"backup_set":str(c.backup_set),"source_commit":c.source_commit,"resolved_stacks":resolved,"postgres_tables":pg,"gitea_tables":gt,"gitea_repositories":gr,"memory_head":mh,"memory_sync_enabled":True,"status":"PASS"}
