...
Unlike the older postgres.py milestone, this verifier consumes the
...
def locate_artifact(backup_set: Path) -> tuple[dict, Path]:
    ...
    if archive.sha256_file(artifact) != matches[0]["sha256"]:
...
def main() -> int:
    ...
    except (PostgresArtifactVerifyError, pg.PostgresVerifyError, planner.RecoveryError, OSError) as exc:
