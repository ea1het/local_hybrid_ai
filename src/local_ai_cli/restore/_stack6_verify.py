...
def verify_stack6_memory(env_path: Path = ROOT / ".env") -> Stack6MemoryVerification:
    values = planner.read_dotenv_presence(env_path)
    base_path = planner.resolve_base_path(values)
    repository = planner.require_env_value(values, "GITMEM_REPOSITORY", label="GITMEM_REPOSITORY")
...
def main() -> int:
    ...
    except (Stack6VerifyError, planner.RecoveryError, OSError) as exc:
