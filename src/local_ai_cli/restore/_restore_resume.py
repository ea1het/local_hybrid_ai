...
def read_env_artifact(backup_set):
    ...
    matches = [a for a in metadata.get("global_artifacts", []) if a.get("resource_id") == "operational-env"]
    if len(matches) != 1:
...
def verify_source(backup_set, stacks_root, source_commit):
    ...
    if cp.returncode != 0:
        raise ResumeError("recorded source lookup failed")
...
def verify_prepared(stacks_root, resolved):
    for sid in resolved:
        if len(list(stacks_root.glob(f"stack{sid}_-*/.lock"))) != 1:
            raise ResumeError(f"stack{sid} is not unambiguously PREPARED")


def postgres_table_count(values):
    db = planner.require_env_value(values, "LITELLM_DB_NAME", label="LITELLM_DB_NAME")
    text = require_ok(
        run(
            [
                "docker",
        ...
        "LiteLLM PostgreSQL verification",
    )
    try:
        count = int(text)
...
def gitea_state(base_path):
    ...
    try:
        ...
        con.close()
    repos = stack4_restore_verify.find_bare_repositories(
        base_path / "service_-_gitea" / "data" / "git" / "repositories"
    )
    for repo in repos:
...
def verify_memory(base_path, values):
    service = planner.require_env_value(values, "HERMES_MEMORY_SERVICE", label="HERMES_MEMORY_SERVICE")
    branch = planner.require_env_value(values, "GITMEM_BRANCH", label="GITMEM_BRANCH")
    ...
    if require_ok(run(prefix + ["status", "--porcelain"]), "portable memory status"):
        raise ResumeError("portable memory working tree is not clean")
    if (
        require_ok(run(prefix + ["rev-parse", f"refs/remotes/origin/{branch}"]), "portable memory remote tracking HEAD")
        != head
    ):
...
def enable_memory_sync(stacks_root, base_path, values):
    ...
    cp = run(
        ...
        cwd=stacks_root / "stack6_-_hermes",
    )
    require_ok(cp, "Stack6 memory-sync profile restore")
    restore_compat.wait_required_runtime(stacks_root, [6], timeout=240)
    if (
        require_ok(run(["docker", "inspect", "-f", "{{.State.Running}}", container]), "memory-sync container status")
        != "true"
    ):
...
def _prepare_resume_context(backup_set, bootstrap):
    ...
    return ResumeContext(
        ...
        bootstrap,
        plan.source_commit,
        tuple(plan.resolved_stacks),
        env_artifact,
        values,
        restore_live._absolute_safe_path(values, "STACKS_ROOT"),
        restore_live._absolute_safe_path(values, "BASE_PATH"),
    )


def resume(backup_set: Path, bootstrap: Path) -> dict[str, object]:
    c = _prepare_resume_context(backup_set, bootstrap)
    verify_source(c.backup_set, c.stacks_root, c.source_commit)
    verify_env(c.env_artifact, c.stacks_root)
    verify_prepared(c.stacks_root, c.resolved_stacks)
    pg = postgres_table_count(c.values)
    gt, gr = gitea_state(c.base_path)
    mh = verify_memory(c.base_path, c.values)
    verify_bootstrap(c.bootstrap, c.base_path, c.values)
    resolved = list(c.resolved_stacks)
    restore_compat.wait_required_runtime(c.stacks_root, resolved, timeout=240)
    restore_compat.install_with_readiness_compat(
        c.stacks_root, resolved, reconcile=True, label="final restore READY/VERIFY/reconcile"
    )
    enable_memory_sync(c.stacks_root, c.base_path, c.values)
    restore_compat.install_with_readiness_compat(
        c.stacks_root, resolved, reconcile=False, label="post-resume final verification"
    )
    return {
        "backup_set": str(c.backup_set),
        "source_commit": c.source_commit,
        "resolved_stacks": resolved,
        "postgres_tables": pg,
        "gitea_tables": gt,
        "gitea_repositories": gr,
        "memory_head": mh,
