...
def sha256_file(path): return dr_archive.sha256_file(path)
...
def validate_completed_backup_set(backup_set):
    ...
    try:
        metadata=json.loads(metadata_path.read_text(encoding="utf-8"));legacy_view=dict(metadata);legacy_view.pop("global_artifacts",None);dr_archive.validate_completed_metadata(legacy_view)
    except (OSError,json.JSONDecodeError,dr_archive.ArchiveBackupError) as exc: raise ArchiveRestoreError(f"invalid backup metadata: {exc}") from exc
