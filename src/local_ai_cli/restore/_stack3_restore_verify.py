...
def verify_checksums(backup_set: Path, metadata: dict) -> None:
    ...
    for relative, digest in expected.items():
        ...
        if archive.sha256_file(path) != digest:
...
def verify_restore(backup_set: Path) -> dict[str, object]:
    ...
    source_db = postgres.validate_identifier(
        planner.require_env_value(values, "LITELLM_DB_NAME", label="LITELLM_DB_NAME"),
        "database",
    )
    app_owner = postgres.validate_identifier(
        planner.require_env_value(values, "LITELLM_DB_USER", label="LITELLM_DB_USER"),
        "role",
    )
    ...
    source_tables = postgres.list_user_tables(source_db)
    if not source_tables:
        raise Stack3RestoreVerifyError("source LiteLLM database contains no user tables")

    restore_db = postgres.validate_identifier(
    ...
    try:
        if postgres.database_exists(restore_db):
        ...
        if cp_create.returncode != 0:
            postgres.fail_command("restore database creation", cp_create)
        created = True

        cp_restore = postgres.run_binary_stdin(
            postgres.pg_restore_database_command(restore_db, app_owner),
            dump_path,
        )
        if cp_restore.returncode != 0:
            postgres.fail_command("pg_restore", cp_restore)

        restored_tables = postgres.list_user_tables(restore_db)
        if restored_tables != source_tables:
            raise Stack3RestoreVerifyError("restored table inventory does not match source database")
        restored_nonempty = postgres.count_nonempty_tables(restore_db, restored_tables)
    finally:
        if created:
            terminate_sql = (
            ...
            cp_terminate = postgres.admin_psql("postgres", terminate_sql)
            if cp_terminate.returncode != 0:
                postgres.fail_command("restore database connection cleanup", cp_terminate)
            cp_drop = postgres.admin_psql(
                "postgres",
                f"DROP DATABASE {postgres.quote_identifier(restore_db)};",
            )
            if cp_drop.returncode != 0:
                postgres.fail_command("restore database removal", cp_drop)

    removed = not postgres.database_exists(restore_db)
    if not removed:
