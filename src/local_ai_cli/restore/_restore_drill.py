...
import _stack6_verify as stack6_verify
...
def _verify_external_prerequisites(backup_set: Path, staged_env: Path) -> int:
    ...
    for prerequisite in metadata.get("prerequisites", []):
        ...
        try:
            stack6_verify.verify_stack6_memory(staged_env)
        except (stack6_verify.Stack6VerifyError, planner.RecoveryError, OSError) as exc:
...
def run_restore_drill(backup_set: Path, destination: Path) -> RestoreDrillResult:
    ...
    stage = restore_stage.stage_restore_all(backup_set, destination)
    try:
        managed = restore_managed.run_managed_restore(backup_set, destination)
