...
def _require_stage(stage: Path) -> tuple[Path, Path, dict[str, str]]:
    ...
    values = planner.read_dotenv_presence(staged_env)
...
def _postgres_image(source_root: Path, values: dict[str, str]) -> str:
    ...
    if not match:
        raise RestoreManagedError("cannot determine Stack3 PostgreSQL image from staged source")
    spec = match.group(1)
    ...
    if expr:
        key, default = expr.groups()
        spec = values.get(key, "").strip() or default
...
def _pg_exec(name: str, database: str, sql: str) -> str:
    cp = _run(
        ["docker", "exec", "-i", name, "psql", "-v", "ON_ERROR_STOP=1", "-At", "-U", "postgres", "-d", database],
        input_bytes=sql.encode(),
    )
    if cp.returncode != 0:
        raise RestoreManagedError(
            "isolated PostgreSQL command failed: " + cp.stderr.decode("utf-8", errors="replace").strip()[:1000]
        )
...
def _postgres_settings(values: dict[str, str]) -> tuple[str, str]:
    ...
    ), _validate_identifier(planner.require_env_value(values, "LITELLM_DB_USER", label="LITELLM_DB_USER"), "role")
...
def _start_isolated_postgres(name: str, image: str, data: Path, password: str) -> None:
    ...
    if cp.returncode != 0:
        raise RestoreManagedError(
            "cannot start isolated PostgreSQL: " + cp.stderr.decode("utf-8", errors="replace").strip()[:1000]
        )
    _wait_postgres(name)
...
def _restore_postgres_dump(name: str, dump: Path, database: str) -> None:
    ...
    if cp.returncode != 0:
        raise RestoreManagedError(
            "isolated PostgreSQL pg_restore failed: " + cp.stderr.decode("utf-8", errors="replace").strip()[:1000]
        )
...
def _postgres_table_stats(name: str, database: str) -> tuple[int, int]:
    ...
    tables = [r for r in rows if r.strip()]
    if not tables:
...
def restore_postgres(
    ...
    try:
        _start_isolated_postgres(name, _postgres_image(source_root, values), data, secrets.token_urlsafe(32))
...
def _render_gitea_config(source_root: Path, values: dict[str, str], target: Path) -> None:
    ...
    replacements = {
        k: planner.require_env_value(values, k, label=k)
        for k in (
            "GITEA_DOMAIN",
            "GITEA_ROOT_URL",
            "GITEA_SSH_DOMAIN",
...
def _wait_gitea(name: str, timeout: int = 120) -> None:
    ...
    while time.monotonic() < deadline:
        if (
            _run(["docker", "exec", name, "wget", "-q", "--spider", "http://127.0.0.1:3000/api/healthz"]).returncode
            == 0
        ):
            return
        ...
        if state.returncode == 0 and state.stdout.decode().strip() in {"exited", "dead"}:
            ...
            raise RestoreManagedError(
                f"isolated Gitea exited before health: {(logs.stdout+logs.stderr).decode('utf-8',errors='replace')[:1500]}"
            )
        time.sleep(1)
    raise RestoreManagedError("isolated Gitea did not become healthy")


def _prepare_gitea_runtime(
    backup_set: Path, stage: Path, source_root: Path, values: dict[str, str], metadata: dict
) -> tuple[Path, Path, int, int, list[Path]]:
    ...
    tables, nonempty = stack4_restore_verify.restore_sqlite(extracted / "gitea-db.sql", data / "gitea.db")
    repositories = stack4_restore_verify.find_bare_repositories(repo_target)
...
def _start_isolated_gitea(name: str, image: str, uid: int, gid: int, config: Path, data: Path) -> None:
    ...
    if cp.returncode != 0:
        raise RestoreManagedError(
            "cannot start isolated Gitea: " + cp.stderr.decode("utf-8", errors="replace").strip()[:1000]
        )
    _wait_gitea(name)
...
def restore_gitea(
    ...
    config, data, tables, nonempty, repositories = _prepare_gitea_runtime(
        backup_set, stage, source_root, values, metadata
    )
    ...
    try:
        _start_isolated_gitea(
            name, planner.require_env_value(values, "GITEA_IMAGE", label="GITEA_IMAGE"), uid, gid, config, data
        )
...
def _prepare_managed_context(backup_set: Path, stage: Path) -> ManagedRestoreContext:
    ...
    if managed.exists():
        raise RestoreManagedError("managed restore target already exists")
...
def run_managed_restore(backup_set: Path, stage: Path) -> ManagedRestoreResult:
    ...
    pg_tables, pg_nonempty = restore_postgres(
        context.backup_set, context.stage, context.source_root, context.values, context.metadata
    )
    g_tables, g_nonempty, g_repos, g_health = restore_gitea(
        context.backup_set, context.stage, context.source_root, context.values, context.metadata
    )
    return ManagedRestoreResult(
        stage=context.stage,
        postgres_tables=pg_tables,
        postgres_nonempty_tables=pg_nonempty,
        gitea_tables=g_tables,
        gitea_nonempty_tables=g_nonempty,
        gitea_repositories=g_repos,
        gitea_health=g_health,
        drill_containers_removed=True,
    )
