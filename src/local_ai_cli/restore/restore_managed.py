...
def _require_stage(stage:Path)->tuple[Path,Path,dict[str,str]]:
    ...
    values=planner.read_dotenv_presence(staged_env); live_base_raw=values.get("BASE_PATH","").strip()
    if live_base_raw:
...
def _verify_legacy_stage_source(source:Path,commit:str)->None:
    cp=subprocess.run(["git","show",f"{commit}:install.py"],cwd=restore_all.PROJECT_ROOT,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=False)
...
def _validate_identifier(value:str,label:str)->str:
    ...
    return value
def _postgres_settings(values:dict[str,str])->tuple[str,str]: return _validate_identifier(planner.require_env_value(values,"LITELLM_DB_NAME",label="LITELLM_DB_NAME"),"database"),_validate_identifier(planner.require_env_value(values,"LITELLM_DB_USER",label="LITELLM_DB_USER"),"role")
...
def _render_gitea_config(source_root:Path,values:dict[str,str],target:Path)->None:
    rendered=(source_root/"stack4_-_gitea"/"config"/"gitea"/"app.ini").read_text(encoding="utf-8"); replacements={k:planner.require_env_value(values,k,label=k) for k in ("GITEA_DOMAIN","GITEA_ROOT_URL","GITEA_SSH_DOMAIN","GITEA_SSH_PORT","GITEA_INTERNAL_TOKEN","GITEA_JWT_SECRET")}
...
def _gitea_identity(values:dict[str,str])->tuple[int,int]:
    uid=int(planner.require_env_value(values,"GITEA_UID",label="GITEA_UID")); gid=int(planner.require_env_value(values,"GITEA_GID",label="GITEA_GID"))
...
def restore_gitea(backup_set:Path,stage:Path,source_root:Path,values:dict[str,str],metadata:dict)->tuple[int,int,int,bool]:
    ...
    try: _start_isolated_gitea(name,planner.require_env_value(values,"GITEA_IMAGE",label="GITEA_IMAGE"),uid,gid,config,data); return tables,nonempty,len(repositories),True
    finally:
        if _docker_exists(name): _remove_container(name)
def _prepare_managed_context(backup_set:Path,stage:Path)->ManagedRestoreContext:
    metadata=restore_all.read_completed_backup_set(backup_set); plan=restore_all.plan_restore_all(backup_set); source_root,_,values=_require_stage(stage); staged_commit=_stage_source_commit(source_root)
