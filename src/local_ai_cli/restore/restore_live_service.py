...
def install_memory_sync_bootstrap(backup_set: Path, source: Path) -> Path:
    ...
    uid = int(planner.require_env_value(values, "HERMES_UID", label="HERMES_UID")); gid = int(planner.require_env_value(values, "HERMES_GID", label="HERMES_GID"))
...
def enable_memory_sync(backup_set: Path, result: dict[str, object]) -> None:
    metadata=restore_all.read_completed_backup_set(backup_set); _,values=read_env_artifact(backup_set,metadata)
    container=planner.require_env_value(values,"MEMORY_SYNC_CONTAINER",label="MEMORY_SYNC_CONTAINER"); stacks_root=Path(str(result["stacks_root"])); stack_dir=stacks_root/"stack6_-_hermes"
...
def execute(backup_set: Path, bootstrap: Path | None = None) -> dict[str, object]:
    ...
    try:
        ...
        source=validate_memory_sync_bootstrap(bootstrap); original=restore_live._restore_external_git
        ...
        try: result=restore_live.execute_restore_all(backup_set,confirm_clean_target=True).as_dict()
        finally: restore_live._restore_external_git=original
