...
def verify_checksums(backup_set: Path) -> None:
    ...
    for relative, digest in expected.items():
        ...
        if archive.sha256_file(target) != digest:
...
def inspect_backup_set(backup_set: Path) -> dict[str, object]:
    ...
    artifact = next(
        ...
        if a.get("stack_id") == 4 and a.get("resource_id") == gitea_archive.GITEA_RESOURCE_ID
    )
    dump_path = backup_set / artifact["relative_path"]
    if not dump_path.is_file() or dump_path.stat().st_size != artifact["size_bytes"]:
        raise Stack4InspectError("Gitea artifact is missing or has wrong size")
    if archive.sha256_file(dump_path) != artifact["sha256"]:
        raise Stack4InspectError("Gitea artifact SHA-256 does not match metadata")
    names = gitea_archive.validate_gitea_dump(dump_path)
...
def main() -> int:
    ...
    except (Stack4InspectError, gitea_archive.GiteaDumpError, archive.ArchiveBackupError, OSError) as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 1
    print(
        json.dumps(result, indent=2, sort_keys=True)
        if args.json
        else f"DR Stack4 Gitea native dump inspection\n- backup set: {result['backup_set']}\n- ZIP members: {result['zip_members']}\n- checksums: PASS\n- ZIP integrity: PASS\n- live runtime modified: no\n- container restarted: no"
    )
    return 0
