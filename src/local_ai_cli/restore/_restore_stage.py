    ...
    try:
        ...
        if cp.returncode != 0:
            raise RestoreStageError(
                f"cannot materialize recorded source commit: {(cp.stderr or '').strip() or 'git archive failed'}"
            )
...
def _prepare_stage_context(backup_set: Path, destination: Path) -> StageContext:
    ...
    return StageContext(
        backup_set, destination, source_root, runtime_root, plan.source_commit, tuple(plan.resolved_stacks), metadata
    )
...
def _verify_external_config(context: StageContext, values: dict[str, str]) -> int:
    ...
    for prereq in context.metadata.get("prerequisites", []):
        ...
        planner.require_env_value(values, key, label=f"recovery prerequisite {prereq['resource_id']}")
        verified += 1
    return verified
