...
def _error(command, code, message):
    return {
...
def json_payload(args=None):
    ...
    except (ComponentInventoryError, OSError) as exc:
...
def _human_stack_id(stack):
    return stack[5:] if stack.startswith("stack") else stack


def cli_text(payload):
    if not payload["success"]:
        return f"INVENTORY ERROR [{payload['error']['code']}]: {payload['error']['message']}"
    headers = ("STACK", "COMPONENT", "TYPE", "CONTAINER", "TRACKED")
    values = [headers]
    for c in payload["components"]:
        values.append(
            (
                _human_stack_id(c["stack"]),
                c["id"],
                c["management_type"],
                c.get("container") or "-",
                "yes" if c["upgrade_visible"] else "no",
            )
        )
    ...
    for label in ("added", "removed", "changed"):
        changed = payload["changes"][label]
        lines.append(f"{label.upper():<8} {', '.join(changed) if changed else '-'}")
    return "\n".join(lines)
