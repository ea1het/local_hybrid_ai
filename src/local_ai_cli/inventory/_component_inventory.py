...
def source_fingerprint():
    ...
    try:
        manifests = _manifests.all_manifests()
    except _manifests.ManifestError as exc:
    ...
    for sid, manifest in sorted(manifests.items()):
        for name in ("manifest.json", "docker-compose.yml"):
            path = ROOT / manifest["directory"] / name
        ...
        "components": [
            {
                "stack": i["stack"],
                "id": i["id"],
                "management_type": i["management"]["type"],
                "service": i.get("service"),
                "container": i.get("container"),
                "upgrade_visible": "upgrade" in i,
            }
            for i in compile_components()
        ],
    }
...
def diff(previous, current):
    old = {
        f"{i['stack']}/{i['id']}": i
        for i in (previous or {}).get("components", [])
        if isinstance(i, dict) and i.get("stack") and i.get("id")
    }
    new = {f"{i['stack']}/{i['id']}": i for i in current["components"]}
    return {
