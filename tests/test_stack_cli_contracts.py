...
from commands import install,runtime_lifecycle,stack_contracts
class StackCliContractTests(unittest.TestCase):
 def test_every_manifest_stack_has_json_payload_contract(self):
  manifests=install.all_manifests();self.assertEqual(sorted(manifests),list(range(8)))
 ...
 def test_stack_contracts_return_objects_not_serialized_text(self):
  for sid,manifest in install.all_manifests().items():self.assertIsInstance(stack_contracts.json_payload(sid,manifest["directory"]),dict)
