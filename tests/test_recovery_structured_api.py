...
class RecoveryStructuredApiTests(unittest.TestCase):
    def test_plan_returns_serializable_public_envelope_without_rendering(self):
        plan = mock.Mock(); plan.as_dict.return_value = {"kind": "local-hybrid-ai-restore-plan", "changes_made": False}
        ...
        self.assertEqual(payload["command"], "restore.plan"); self.assertTrue(payload["success"]); self.assertFalse(payload["result"]["changes_made"])
    ...
    def test_plan_failure_is_stable_public_envelope(self):
        ...
        with mock.patch.object(public_api, "_load", return_value=engine): payload = public_api.plan_payload("/backup/set")
        self.assertFalse(payload["success"]); self.assertEqual(payload["command"], "restore.plan"); self.assertEqual(payload["error"]["code"], "RESTORE_PLAN_FAILED"); self.assertEqual(payload["error"]["message"], "bad recovery point")
    ...
    def test_clean_target_preflight_is_structured(self):
        ...
        with mock.patch.object(public_api, "_load_script", return_value=entry): payload = public_api.check_clean_target_payload("/backup/set")
        self.assertEqual(payload["command"], "restore.apply"); self.assertFalse(payload["result"]["changes_made"]); entry.check_clean.assert_called_once()
    ...
    def test_live_apply_is_structured(self):
        ...
        with mock.patch.object(public_api, "_load_script", return_value=entry): payload = public_api.apply_payload("/backup/set", "/operator/ssh")
        self.assertEqual(payload["command"], "restore.apply"); entry._execute_with_optional_bootstrap.assert_called_once()
    ...
    def test_live_apply_failure_is_stable_public_envelope(self):
        ...
        with mock.patch.object(public_api, "_load_script", return_value=entry): payload = public_api.apply_payload("/backup/set")
    ...
    def test_resume_is_structured(self):
        ...
        with mock.patch.object(public_api, "_load_script", return_value=entry): payload = public_api.resume_payload("/backup/set", "/operator/ssh")
        self.assertEqual(payload["command"], "restore.resume"); self.assertTrue(payload["result"]["memory_sync_enabled"])
    ...
    def test_plan_public_dispatch_does_not_spawn_private_script(self):
        ...
        with mock.patch.object(cli.recovery_api, "plan_payload", return_value=payload) as plan, mock.patch.object(cli, "_run_internal") as internal, mock.patch.object(cli.render, "render_json"):
            rc = cli.restore_command(["plan", "/backup/set"], cli.CLIContext(json_output=True))
        self.assertEqual(rc, 0); plan.assert_called_once_with("/backup/set"); internal.assert_not_called()
    ...
    def test_drill_public_dispatch_does_not_spawn_private_script(self):
        ...
        with mock.patch.object(cli.recovery_api, "drill_payload", return_value=payload) as drill, mock.patch.object(cli, "_run_internal") as internal, mock.patch.object(cli.render, "render_cli"):
            rc = cli.restore_command(["drill", "/backup/set", "--destination", "/tmp/drill"], cli.CLIContext(assume_yes=True))
        self.assertEqual(rc, 0); drill.assert_called_once_with("/backup/set", "/tmp/drill"); internal.assert_not_called()

    def test_public_dispatcher_has_no_private_json_rewrapper(self):
        source = (cli.ROOT / "commands" / "cli.py").read_text(encoding="utf-8")
        self.assertNotIn("def _run_internal_json", source)
