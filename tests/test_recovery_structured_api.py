...
class RecoveryStructuredApiTests(unittest.TestCase):
 def test_backup_returns_serializable_public_envelope_without_rendering(self):
  ...
  with mock.patch.object(backup_api,"_load",side_effect=[dr,engine]):payload=backup.backup_payload("/backup")
 ...
 def test_plan_returns_serializable_public_envelope_without_rendering(self):
  ...
  with mock.patch.object(restore_api,"_load",return_value=engine):payload=restore.plan_payload("/backup/set")
 ...
 def test_drill_returns_serializable_public_envelope_without_rendering(self):
  ...
  with mock.patch.object(restore_api,"_load",return_value=engine):payload=restore.drill_payload("/backup/set","/tmp/drill")
 ...
 def test_clean_target_preflight_uses_importable_service(self):
  ...
  load.assert_called_once_with("_restore_live_service");self.assertTrue(payload["success"])
 ...
 def test_resume_uses_importable_engine(self):
  ...
  load.assert_called_once_with("_restore_resume");self.assertTrue(payload["success"])
