...
class RecoveryStructuredApiTests(unittest.TestCase):
 def test_backup_returns_serializable_public_envelope_without_rendering(self):
  result=mock.Mock();result.as_dict.return_value={"path":"/backup/set","artifact_count":3};dr=mock.Mock();dr.resolve_backup_root.return_value=("/backup",None);engine=mock.Mock();engine.execute_backup_all.return_value=result
  with mock.patch.object(backup,"_load",side_effect=[dr,engine]):payload=backup.backup_payload("/backup")
 ...
 def test_plan_returns_serializable_public_envelope_without_rendering(self):
  ...
  with mock.patch.object(restore,"_load",return_value=engine):payload=restore.plan_payload("/backup/set")
 ...
 def test_drill_returns_serializable_public_envelope_without_rendering(self):
  ...
  with mock.patch.object(restore,"_load",return_value=engine):payload=restore.drill_payload("/backup/set","/tmp/drill")
 ...
 def test_clean_target_preflight_uses_importable_service(self):
  ...
  with mock.patch.object(restore,"_load",return_value=service) as load:payload=restore.check_clean_target_payload("/backup/set")
 ...
 def test_live_apply_failure_is_stable_public_envelope(self):
  ...
  with mock.patch.object(restore,"_load",return_value=service):payload=restore.apply_payload("/backup/set")
 ...
 def test_resume_uses_importable_engine(self):
  ...
  with mock.patch.object(restore,"_load",return_value=engine) as load:payload=restore.resume_payload("/backup/set","/operator/ssh")
 ...
 def test_public_dispatcher_has_no_recovery_facade_or_private_subprocess_boundary(self):
  source=(cli.ROOT/"commands"/"cli.py").read_text(encoding="utf-8");self.assertNotIn("commands.recovery",source);self.assertNotIn("recovery_api",source);self.assertNotIn("_run_internal",source);self.assertNotIn("subprocess",source)
 def test_owned_apis_contain_no_rendering_or_subprocess_boundary(self):
  for path in (backup.PACKAGE_ROOT/"api.py",restore.RESTORE_ROOT/"api.py"):
