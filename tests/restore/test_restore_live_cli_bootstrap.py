...
from commands.restore import dr_restore_live_service as restore_service

class RestoreLiveBootstrapTests(unittest.TestCase):
    def _bootstrap(self,root:Path)->Path:
        source=root/"bootstrap";source.mkdir()
        for name in ("ssh_config","id_ed25519","known_hosts"):(source/name).write_text(f"{name}\n",encoding="utf-8")
        return source
    def test_global_operational_env_uses_resource_id_contract(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);env=root/"artifacts"/"global"/"operational.env";env.parent.mkdir(parents=True);env.write_text("STACKS_ROOT=/opt/docker/stacks\nBASE_PATH=/opt/docker/runtime\n");metadata={"global_artifacts":[{"resource_id":"operational-env","relative_path":"artifacts/global/operational.env"}]};path,values=restore_service.read_env_artifact(root,metadata);self.assertEqual(path,env);self.assertEqual(values["STACKS_ROOT"],"/opt/docker/stacks")
    ...
    def test_validate_rejects_missing_required_material(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(restore_service.BootstrapError):restore_service.validate_memory_sync_bootstrap(source)
    def test_install_bootstrap_copies_private_material_to_clean_runtime_target(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            for name in ("ssh_config","id_ed25519","known_hosts"):
                path=target/name;self.assertTrue(path.is_file());self.assertEqual(stat.S_IMODE(path.stat().st_mode),0o600)
