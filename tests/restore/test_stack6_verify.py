...
class Stack6VerifyTests(unittest.TestCase):
    def init_repo(self, root):
        ...
        mod.subprocess.run(
            ["git", "-C", str(tree), "remote", "add", "origin", "ssh://git@gitea/example/hermes-memory.git"], check=True
        )
        head = mod.subprocess.run(
            ["git", "-C", str(tree), "rev-parse", "HEAD"], check=True, text=True, stdout=mod.subprocess.PIPE
        ).stdout.strip()
    ...
    def test_verifier_accepts_clean_aligned_memory_repo(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            result = mod.verify_stack6_memory(self.write_env(root))
    ...
    def test_soul_is_not_required(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            self.init_repo(root)
            self.assertNotIn("SOUL.md", mod.verify_stack6_memory(self.write_env(root)).required_files)
    ...
    def test_verifier_rejects_untracked_required_file(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            env = self.write_env(root)
            mod.subprocess.run(
                ["git", "-C", str(tree), "rm", "--cached", "USER.md"], check=True, stdout=mod.subprocess.DEVNULL
            )
            with self.assertRaises(mod.Stack6VerifyError):
                mod.verify_stack6_memory(env)
