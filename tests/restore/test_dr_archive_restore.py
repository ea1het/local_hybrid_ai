...
from local_ai_cli.common import dr_archive as backup_archive
from local_ai_cli.restore import dr_archive_restore
class DisasterRecoveryArchiveRestoreTests(unittest.TestCase):
 ...
 def test_restore_verification_matches_source_and_cleans_temp(self):
  with tempfile.TemporaryDirectory() as tmp:
   parent=Path(tmp);source=self.make_source(parent);result=dr_archive_restore.verify_restore(self.make_backup(parent,source),compare_source=source);self.assertTrue(result.source_match);self.assertTrue(result.temporary_cleanup);self.assertEqual(result.member_count,3)
 def test_checksum_tampering_is_rejected(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   with self.assertRaises(dr_archive_restore.ArchiveRestoreError):dr_archive_restore.verify_restore(backup,compare_source=source)
 ...
 def test_path_traversal_member_name_is_rejected(self):
  for name in ("../escape","/absolute","other/file"):
   with self.assertRaises(dr_archive_restore.ArchiveRestoreError):dr_archive_restore.safe_member_name(name)
 def test_absolute_symlink_target_is_rejected(self):
  with self.assertRaises(dr_archive_restore.ArchiveRestoreError):dr_archive_restore.validate_symlink_target(dr_archive_restore.PurePosixPath("pki/link"),"/etc/passwd")
 def test_result_json_never_claims_live_restore(self):
  with tempfile.TemporaryDirectory() as tmp:
   parent=Path(tmp);source=self.make_source(parent);payload=dr_archive_restore.verify_restore(self.make_backup(parent,source),compare_source=source).as_dict();self.assertFalse(payload["live_runtime_modified"]);self.assertTrue(payload["temporary_cleanup"]);self.assertTrue(payload["source_match"]);json.dumps(payload)
