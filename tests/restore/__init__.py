...
each other by short flat name (e.g. ``import stack4_backup``), relying on
...
# backup is inserted last so its stack4_backup.py (which restore's own
# stack4_inspect.py depends on for GITEA_RESOURCE_ID) wins the ambiguous name.
