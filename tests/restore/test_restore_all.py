...
class RestoreAllPlannerTests(unittest.TestCase):
    ...
    def lifecycle(self):
        ...
        return {
            "schema_version": 1,
            "stacks": {str(sid): {"directory": manifest["directory"]} for sid, manifest in manifests.items()},
        }
    ...
    def test_manifest_strategy_drift_is_rejected(self):
        ...
        with self.assertRaises(restore_all.RestoreAllError):
            restore_all.validate_manifest_correspondence(self.metadata(), manifests)

    def test_missing_operational_env_global_artifact_is_rejected(self):
        metadata = self.metadata()
        metadata["global_artifacts"] = []
        with self.assertRaises(restore_all.RestoreAllError):
            restore_all.validate_manifest_correspondence(metadata, self.manifests())
    ...
    def test_checksum_index_requires_exact_declared_coverage(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(restore_all.RestoreAllError):
                restore_all.verify_checksum_index(root, metadata)
    ...
    def test_plan_is_read_only_payload(
        self, read_meta, verify_checksums, load_manifests, load_lifecycle, git_available
    ):
