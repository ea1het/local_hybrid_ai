...
class DisasterRecoveryArchiveRestoreTests(unittest.TestCase):
    ...
    def make_backup(self, parent, source):
        ...
        os.chmod(root, 0o700)
        artifact = backup_archive.ArchiveArtifact(
            stack_id=0,
        ...
        return backup_archive.execute_archive_backup_set(
            backup_root=root,
            source=source,
            source_commit="a" * 40,
            requested=["0"],
    ...
    def test_restore_verification_matches_source_and_cleans_temp(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = archive_restore.verify_restore(self.make_backup(parent, source), compare_source=source)
    ...
    def test_checksum_tampering_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            parent = Path(tmp)
            source = self.make_source(parent)
            backup = self.make_backup(parent, source)
            artifact = backup / "artifacts/stack0/platform-pki.tar"
            with artifact.open("ab") as h:
                h.write(b"tamper")
    ...
    def test_path_traversal_member_name_is_rejected(self):
        for name in ("../escape", "/absolute", "other/file"):
            with self.assertRaises(archive_restore.ArchiveRestoreError):
                archive_restore.safe_member_name(name)
    ...
    def test_result_json_never_claims_live_restore(self):
        with tempfile.TemporaryDirectory() as tmp:
            parent = Path(tmp)
            source = self.make_source(parent)
            payload = archive_restore.verify_restore(self.make_backup(parent, source), compare_source=source).as_dict()
