...
class VersionAuthorityTests(unittest.TestCase):
    def test_selectability_matches_runtime_qualification_state(self):
        ...
        exceptions = {
            "stack0/platform-foundation": "non-versioned-component",
            "stack4/runner": "local-managed",
            "stack6/sandbox": "local-build",
        }
        for key, record in records.items():
            if key in exceptions:
                self.assertFalse(record.get("selectable", True), key)
                self.assertEqual(record["execution"]["blocked_by"], exceptions[key], key)
            else:
                self.assertTrue(record.get("selectable", True), key)
                self.assertEqual(record["execution"], {"mode": "guarded", "blocked_by": None}, key)

    def test_catalog_no_longer_uses_tracked_compose_pin_as_block_reason(self):
        for key, record in upgrade.component_records().items():
            self.assertNotEqual(record["execution"].get("blocked_by"), "tracked-compose-pin", key)

    def test_compose_baselines_pin_pre_adoption_runtime_versions(self):
        root = Path(__file__).resolve().parents[2]
        s1 = (root / "stack1_-_haproxy_web" / "docker-compose.yml").read_text()
        s2 = (root / "stack2_-_searxng_firecrawl" / "docker-compose.yml").read_text()
        s5 = (root / "stack5_-_dockhand" / "docker-compose.yml").read_text()
        self.assertIn("${HAPROXY_VERSION:-3.0.26-alpine3.24}", s1)
        self.assertIn("${FIRECRAWL_REDIS_VERSION:-8.10.0-alpine3.23}", s2)
        self.assertIn("${FIRECRAWL_RABBITMQ_VERSION:-3.13.7-alpine}", s2)
        self.assertIn("${DOCKHAND_REPOSITORY:-fnsys/dockhand}:${DOCKHAND_VERSION:-v1.0.40}", s5)
        self.assertNotIn("image: haproxy:3.0-alpine", s1)
        self.assertNotIn("image: redis:alpine", s2)
        self.assertNotIn("image: rabbitmq:3-alpine", s2)
    ...
    def test_exact_split_identity_does_not_need_registry_lookup(self):
        ...
        with mock.patch.object(upgrade_adopt.upgrade_registry, "inspect") as inspect:
            repository, version = upgrade_adopt._split_identity(c, "ghcr.io/open-webui/open-webui:v0.11.3")
        self.assertEqual((repository, version), ("ghcr.io/open-webui/open-webui", "v0.11.3"))
        inspect.assert_not_called()

    def test_tracking_split_identity_resolves_exact_running_version(self):
        c = upgrade.Component("stack2", "redis", "firecrawl-redis", "firecrawl-redis", None, None)
        ...
        with mock.patch.object(upgrade_adopt.upgrade_registry, "inspect", return_value=state):
            repository, version = upgrade_adopt._split_identity(c, "redis:alpine")
    ...
    def test_selection_baseline_resolves_historical_tracking_runtime(self):
        c = upgrade.Component(
            ...
            "firecrawl-redis",
            "firecrawl-redis",
            "stack2_-_searxng_firecrawl/docker-compose.yml",
            "redis/redis",
        )
        state = upgrade_registry.RegistryState(
            image="redis:alpine",
        ...
        with (
            mock.patch.object(upgrade_selection.upgrade, "running_image", return_value="redis:alpine"),
            mock.patch.object(upgrade_selection.upgrade_registry, "inspect", return_value=state),
        ):
            current = upgrade_selection.current_runtime_version(c, {})
    ...
    def test_selection_baseline_fails_closed_to_literal_when_registry_resolution_fails(self):
        ...
        with (
            mock.patch.object(upgrade_selection.upgrade, "running_image", return_value="redis:alpine"),
            mock.patch.object(
                upgrade_selection.upgrade_registry,
                "inspect",
    ...
    def test_adoption_appends_missing_authority_without_rewriting_existing_values(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            written = upgrade_adopt._apply_missing(
                path, {"HAPROXY_IMAGE": "haproxy", "HAPROXY_VERSION": "3.0.26-alpine3.24"}
            )
            text = path.read_text()
    ...
    def test_adoption_conflict_fails_without_mutating_env(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(upgrade_adopt.AdoptionError) as ctx:
                upgrade_adopt._apply_missing(path, {"FIRECRAWL_REDIS_VERSION": "8.10.0-alpine3.23"})
