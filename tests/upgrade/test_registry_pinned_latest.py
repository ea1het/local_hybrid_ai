...
class RegistryPinnedLatestTests(unittest.TestCase):
    def test_digest_label_uses_first_nine_sha_characters(self):
        self.assertEqual(container_registry.digest_label("sha256:df1a393ce8bfc3801570"), "latest(df1a393ce)")
    ...
    def test_digest_only_latest_only_package_reports_current_with_digest_label(self):
        ...
        with (
            mock.patch(P + ".local_digest", return_value="sha256:same111111"),
            mock.patch(P + ".local_version_hint", return_value=None),
            mock.patch(P + ".registry_tags", return_value=tags),
            mock.patch(
                P + ".manifest_probe",
                side_effect=lambda ref: (
                    container_registry.RemoteProbe("sha256:same111111", "ok")
                    if ref.endswith(":latest")
                    else container_registry.RemoteProbe(None, "not_found")
                ),
            ),
        ):
    ...
    def test_digest_only_latest_only_package_reports_newer_digest_by_publication_time(self):
        ...
        tags = container_registry.TagProbe(("latest", "linux-amd64", "linux-arm64"), "ok")
        pub = {
            image: container_registry.PublicationProbe("sha256:old111111111", "ok", "2026-09-01T10:00:00Z"),
            "ghcr.io/firecrawl/nuq-postgres:latest": container_registry.PublicationProbe(
                "sha256:new222222222", "ok", "2026-09-12T10:00:00Z"
            ),
        }
        with (
            mock.patch(P + ".local_digest", return_value="sha256:old111111111"),
            mock.patch(P + ".local_version_hint", return_value=None),
            mock.patch(P + ".registry_tags", return_value=tags),
            mock.patch(P + ".manifest_probe", return_value=container_registry.RemoteProbe("sha256:new222222222", "ok")),
            mock.patch(P + ".manifest_publication", side_effect=lambda ref: pub[ref]),
        ):
    ...
    def test_digest_only_latest_only_package_rejects_older_remote_digest(self):
        ...
        pub = {
            image: container_registry.PublicationProbe("sha256:new222222222", "ok", "2026-09-12T10:00:00Z"),
            "ghcr.io/firecrawl/nuq-postgres:latest": container_registry.PublicationProbe(
                "sha256:old111111111", "ok", "2026-09-01T10:00:00Z"
            ),
        }
        with (
            mock.patch(P + ".local_digest", return_value="sha256:new222222222"),
            mock.patch(P + ".local_version_hint", return_value=None),
            mock.patch(P + ".registry_tags", return_value=tags),
            mock.patch(P + ".manifest_probe", return_value=container_registry.RemoteProbe("sha256:old111111111", "ok")),
            mock.patch(P + ".manifest_publication", side_effect=lambda ref: pub[ref]),
        ):
            state = container_registry.inspect("firecrawl-postgres", image)
        self.assertEqual(state.available_version, "latest(new222222)")
    ...
    def test_digest_only_latest_only_package_fails_closed_without_dates(self):
        ...
        with (
            ...
            mock.patch(P + ".registry_tags", return_value=tags),
            mock.patch(P + ".manifest_probe", return_value=container_registry.RemoteProbe("sha256:new222222222", "ok")),
            mock.patch(P + ".manifest_publication", return_value=container_registry.PublicationProbe(None, "ok", None)),
        ):
            state = container_registry.inspect("firecrawl-postgres", image)
    ...
    def test_manifest_publication_prefers_registry_last_modified(self):
        with mock.patch(
            ...
            return_value=(
                200,
                {"Docker-Content-Digest": "sha256:abc", "Last-Modified": "Sat, 12 Sep 2026 10:00:00 GMT"},
                b"{}",
            ),
        ):
            probe = container_registry.manifest_publication("ghcr.io/firecrawl/playwright-service:latest")
        ...
        def manifest(ref):
            return container_registry.RemoteProbe(
                (
                    ...
                    else ("sha256:old" if ref.endswith(":2.11.310") else "sha256:new")
                ),
                "ok",
            )

        with (
            mock.patch(P + ".local_digest", return_value="sha256:old"),
            mock.patch(P + ".local_version_hint", return_value=None),
            mock.patch(P + ".registry_tags", return_value=tags),
            mock.patch(P + ".manifest_probe", side_effect=manifest),
        ):
            state = container_registry.inspect("firecrawl-api", image)
        self.assertEqual((state.current_version, state.available_version), ("2.11.310", "2.11.332"))
        self.assertTrue(state.update_available)
