...
class UpgradeAdoptPartialTests(unittest.TestCase):
 def test_unprepared_missing_component_is_skipped(self):
  ...
  with mock.patch.dict(upgrade_adopt.AUTHORITIES,{"stack1/haproxy":{"type":"split","image_key":"HAPROXY_IMAGE","version_key":"HAPROXY_VERSION"}},clear=True),mock.patch("local_ai_cli.upgrade._adopt.upgrade.load_catalog",return_value=[component]),mock.patch("local_ai_cli.upgrade._adopt.upgrade.running_image",return_value=None),mock.patch("local_ai_cli.upgrade._adopt._component_prepared",return_value=False):updates,records=upgrade_adopt.desired_updates()
 ...
 def test_prepared_component_without_runtime_identity_fails_closed(self):
  component=SimpleNamespace(stack="stack1",name="haproxy",compose="stack1_-_haproxy_web/docker-compose.yml",container="haproxy")
  with mock.patch.dict(upgrade_adopt.AUTHORITIES,{"stack1/haproxy":{"type":"split","image_key":"HAPROXY_IMAGE","version_key":"HAPROXY_VERSION"}},clear=True),mock.patch("local_ai_cli.upgrade._adopt.upgrade.load_catalog",return_value=[component]),mock.patch("local_ai_cli.upgrade._adopt.upgrade.running_image",return_value=None),mock.patch("local_ai_cli.upgrade._adopt._component_prepared",return_value=True):
 ...
 def test_domain_payload_does_not_render_or_serialize(self):
  ...
  with mock.patch("local_ai_cli.upgrade._adopt.upgrade.ROOT") as root,mock.patch("local_ai_cli.upgrade._adopt.desired_updates",return_value=({},[])),mock.patch("local_ai_cli.upgrade._adopt._read_operational_env",return_value={}):
