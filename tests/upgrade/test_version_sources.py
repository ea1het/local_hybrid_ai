...
class VersionSourceTests(unittest.TestCase):
    ...
    def _inventory(self, component, record, image, *, online=True, registry_state=None):
        ...
        with (
            ...
            mock.patch.object(upgrade, "load_catalog", return_value=[component]),
            mock.patch.object(upgrade, "component_records", return_value=records),
            mock.patch.object(upgrade, "read_env", return_value={}),
            mock.patch.object(upgrade, "load_plan", return_value={"schema_version": 1, "selected": {}}),
            mock.patch.object(upgrade, "running_image", return_value=image),
            mock.patch.object(upgrade.upgrade_registry, "inspect", return_value=registry_state) as registry,
        ):
    ...
    def test_manifest_upgrade_metadata_uses_no_lateral_version_sources(self):
        ...
        components = [c for s in catalog["stacks"] for c in s["components"]]
        self.assertFalse(any("version_source" in c for c in components))
        self.assertFalse(any("registry_source" in c for c in components))
    ...
    def test_inventory_reports_registry_human_version_update(self):
        state = container_registry.RegistryState(
            ...
            remote_status="ok",
            current_version="3.0.18-alpine",
            available_version="3.0.19-alpine",
            tags_status="ok",
            registry="docker.io",
            repository="library/haproxy",
        )
        rows, _ = self._inventory(
            self._inventory_component(),
            {"stack": "stack1", "id": "haproxy"},
            "haproxy:3.0-alpine",
    ...
    def test_inventory_offline_does_not_query_registry(self):
        rows, registry = self._inventory(
            self._inventory_component(), {"stack": "stack1", "id": "haproxy"}, "haproxy:3.0-alpine", online=False
        )
    ...
    def test_digest_pin_uses_discovered_human_registry_version(self):
        ...
        rows, registry = self._inventory(
            component, {"stack": "stack2", "id": "firecrawl"}, state.image, registry_state=state
        )
    ...
    def test_exact_tag_component_also_uses_its_image_registry(self):
        ...
        rows, registry = self._inventory(
            component, {"stack": "stack7", "id": "open-webui"}, state.image, registry_state=state
        )
    ...
    def test_local_component_is_not_queried_against_registry(self):
        component = upgrade.Component(
            ...
            compose="stack6_-_hermes/docker-compose.yml",
            upstream=None,
            selectable=False,
        )
        rows, registry = self._inventory(
