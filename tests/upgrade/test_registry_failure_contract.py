...
class RegistryFailureContractTests(unittest.TestCase):
 ...
 def test_registry_tags_preserve_rate_limit_and_auth_failures(self):
  for status,expected in ((429,"rate_limited"),(401,"unauthorized"),(403,"forbidden")):
   with self.subTest(status=status),mock.patch("local_ai_cli.upgrade._registry._registry_request",return_value=(status,{},b"")):
 ...
 def test_inventory_never_renders_registry_failure_as_current(self):
  component=upgrade.Component(stack="stack1",name="haproxy",service=None,container="haproxy",compose=None,upstream="haproxy/haproxy",selectable=True);record={"stack":"stack1","id":"haproxy"}
