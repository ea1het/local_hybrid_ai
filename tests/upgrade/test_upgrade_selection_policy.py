...
from local_ai_cli.upgrade import config as upgrade_core_impl
from local_ai_cli.upgrade import api as upgrade_entry


class UpgradeSelectionPolicyTests(unittest.TestCase):
    def _runtime_patches(self, runtime):
        return (
            mock.patch.object(upgrade_selection.upgrade, "runtime_root", return_value=runtime),
            mock.patch.object(
                upgrade_selection.upgrade, "running_image", return_value="ghcr.io/open-webui/open-webui:v0.11.3"
            ),
            mock.patch.object(upgrade_core_impl, "runtime_root", return_value=runtime),
            mock.patch.object(upgrade_core_impl, "running_image", return_value="ghcr.io/open-webui/open-webui:v0.11.3"),
        )

    def _select(self, stack, name, version):
        return upgrade_entry.select_payload(stack, name, version)

    def test_select_persists_newer_target_inside_minor_series(self):
        digest = "sha256:" + "a" * 64
        with tempfile.TemporaryDirectory() as tmp:
            runtime = Path(tmp)
            p1, p2, p3, p4 = self._runtime_patches(runtime)
            with (
                p1,
                p2,
                p3,
                p4,
                mock.patch.object(
                    container_registry, "manifest_probe", return_value=container_registry.RemoteProbe(digest, "ok")
                ),
            ):
                payload = self._select("stack7", None, "v0.11.4")
            self.assertTrue(payload["success"])
            selected = json.loads((runtime / "platform" / "upgrade-plan.json").read_text())["selected"][
                "stack7/open-webui"
            ]
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                with self.assertRaises(upgrade.UpgradeError) as ctx:
                    self._select("stack7", None, "v0.12.0")
    ...
    def test_select_rejects_target_missing_from_registry(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                p4,
                mock.patch.object(
                    container_registry, "manifest_probe", return_value=container_registry.RemoteProbe(None, "not_found")
                ),
            ):
                with self.assertRaises(upgrade.UpgradeError) as ctx:
                    self._select("stack7", None, "v0.11.99")
    ...
    def test_manual_override_allows_explicit_newer_target_outside_minor_series(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                payload = self._select("stack7", None, "v0.12.0")
            self.assertTrue(payload["success"])
            self.assertEqual(
                json.loads((runtime / "platform" / "upgrade-plan.json").read_text())["selected"]["stack7/open-webui"][
                    "policy_at_selection"
                ],
                "manual",
            )
    ...
    def test_apply_validation_rejects_a_tag_that_moved_after_selection(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                with self.assertRaises(upgrade.UpgradeError) as ctx:
                    upgrade_selection.validate_selected_baselines([selection])
