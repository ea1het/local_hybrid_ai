...
class UpgradeGuardTests(unittest.TestCase):
    ...
    def test_apply_detection_is_position_independent_for_global_flags(self):
        for argv in (
            ...
            self.assertTrue(upgrade_guard._is_upgrade_apply(list(argv)), argv)
        for argv in (["upgrade"], ["upgrade", "check", "--yes"], ["upgrade", "policy", "7", "--yes"]):
            self.assertFalse(upgrade_guard._is_upgrade_apply(list(argv)), argv)
        ...
        with tempfile.TemporaryDirectory() as tmp, mock.patch.dict(os.environ, {"LOCAL_AI_RUNTIME_ROOT": tmp}):
            ...
            with lock_path.open("a+") as held:
                ...
                with redirect_stdout(output):
                    rc = upgrade_guard.run_guarded(["--yes", "upgrade", "--json"], lambda: 0)
    ...
    def test_failed_apply_is_journaled_with_selection_and_error_code(self):
        with tempfile.TemporaryDirectory() as tmp, mock.patch.dict(os.environ, {"LOCAL_AI_RUNTIME_ROOT": tmp}):
            platform = Path(tmp) / "platform"
            platform.mkdir(parents=True)
            (platform / "upgrade-plan.json").write_text(
                json.dumps(
                    {
                        "schema_version": 1,
                        "selected": {
