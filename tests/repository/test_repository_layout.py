...
class RepositoryLayoutTests(unittest.TestCase):
 def test_python_tests_live_under_the_tests_tree(self):
  files=tracked_files();offenders=[]
  for item in files:
   path=Path(item);parts=path.parts
   if not path.name.startswith("test_") or path.suffix!=".py":continue
   if parts[0]!="tests":offenders.append(item);continue
 ...
 def test_dr_tests_do_not_shadow_production_dr_module(self):
  files=tracked_files();self.assertNotIn("tests/dr/dr.py",files);self.assertTrue(any(p.startswith("tests/backup/") or p.startswith("tests/restore/") or p.startswith("tests/common/") for p in files));self.assertIn("src/local_ai_cli/backup/_planner.py",files);self.assertFalse(any(p.startswith("commands/") for p in files))
 def test_documentation_and_decision_roots_are_normalized(self):
  ...
  files=tracked_files()
  for prefix in ("adr/","sdr/","openspec/","ADRs/"):self.assertFalse(any(p.startswith(prefix) for p in files))
  for item in ("INSTALLATION.md","a2aknowledge.md","pending.md"):self.assertNotIn(item,files)
 def test_long_form_markdown_lives_under_docs(self):
  offenders=[]
  for item in tracked_files():
   path=Path(item)
   if path.suffix!=".md" or (path.parts and path.parts[0]=="docs") or path.name=="README.md":continue
   offenders.append(item)
  self.assertEqual(sorted(offenders),[])
 def test_recovery_schemas_are_owned_by_command_packages(self):
  files=tracked_files();self.assertNotIn("recovery.schema.json",files);self.assertIn("src/local_ai_cli/restore/recovery.schema.json",files);self.assertIn("src/local_ai_cli/common/backup-set.schema.json",files);self.assertFalse(any(p.startswith("commands/") for p in files))
 def test_historical_uppercase_document_names_do_not_return(self):
  forbidden={"INSTALLATION.md","STATUS.md","MCP.md","INTEGRATIONS.md"};self.assertEqual([p for p in tracked_files() if Path(p).name in forbidden],[])
 ...
 def test_local_ai_is_the_supported_root_management_cli(self):
  ...
  for package in ("install","backup","restore","status","doctor","inventory","completion","upgrade","lifecycle"):self.assertTrue(any(p.startswith(f"src/local_ai_cli/{package}/") for p in files),package)
  self.assertIn("src/local_ai_cli/install-lifecycle.json",files);self.assertNotIn("src/local_ai_cli/install.py",files);self.assertFalse(any(p.startswith("commands/") for p in files));self.assertFalse(any(p.startswith("internal/") for p in files));self.assertFalse(any(p.startswith("installer/") for p in files));self.assertFalse(any(p.startswith("bkp-dr/") for p in files));self.assertNotIn("install.py",files)
