...
from local_ai_cli import doctor
class DoctorTests(unittest.TestCase):
 ...
 def test_run_checks_reports_required_prerequisites_without_mutation(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   with mock.patch.object(doctor.api,"ROOT",root),mock.patch.dict("os.environ",{"LOCAL_AI_RUNTIME_ROOT":str(runtime)},clear=False),mock.patch("local_ai_cli.doctor.api.install.all_manifests",return_value={0:{"directory":"stack0"}}),mock.patch("local_ai_cli.doctor.api.install.load_lifecycle",return_value={"stacks":{"0":{}}}),mock.patch("local_ai_cli.doctor.api.install.validate_registry") as validate,mock.patch("local_ai_cli.doctor.api.component_inventory.compile_components",return_value=[{"stack":"stack0","id":"platform-foundation"}]) as components,mock.patch("local_ai_cli.doctor.api.shutil.which",return_value="/usr/bin/docker"),mock.patch("local_ai_cli.doctor.api.subprocess.run",return_value=compose):checks=doctor.run_checks()
 ...
 def test_json_main_returns_machine_contract(self):
  ...
  with mock.patch("local_ai_cli.doctor.api.run_checks",return_value=checks),mock.patch("builtins.print") as output:rc=doctor.main(json_output=True)
