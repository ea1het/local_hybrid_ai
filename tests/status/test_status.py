...
class StatusOperationalTests(unittest.TestCase):
 ...
 def test_stack_inventory_contains_only_operational_state(self):
  ...
  with mock.patch("commands.status.api.install.all_manifests",return_value=manifests),mock.patch("commands.status.api.install.load_lifecycle",return_value=lifecycle),mock.patch("commands.status.api.install.validate_registry"),mock.patch("commands.status.api.install.stack_state",return_value={"prepared":True,"containers":{"dockhand":"running/healthy"}}):rows=status.stack_inventory()
 ...
 def test_json_status_contains_no_upgrade_state(self):
  ...
  with mock.patch("commands.status.api.stack_inventory",return_value=stacks):payload=status.json_payload()
