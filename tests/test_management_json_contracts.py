...
class ManagementJsonContractTests(unittest.TestCase):
 def _install_boundary(self,argv):
  ...
  with mock.patch.object(install.api.engine,"preflight"),mock.patch.object(install.api.engine,"all_manifests",return_value=manifests),mock.patch.object(install.api.engine,"load_lifecycle",return_value=lifecycle),mock.patch.object(install.api.engine,"validate_registry"),mock.patch.object(install.api.engine,"resolve_requested",return_value=[7]),mock.patch.object(install.api.engine,"resolve_plan",return_value=[7]),mock.patch.object(install.api.engine,"build_actions",return_value=([],[],set())):return install.build_payload(argv)
 def test_install_plan_json_exposes_high_level_lifecycle_without_private_commands(self):
  ...
  with mock.patch.object(install.api.engine,"preflight"),mock.patch.object(install.api.engine,"all_manifests",return_value=manifests),mock.patch.object(install.api.engine,"load_lifecycle",return_value=lifecycle),mock.patch.object(install.api.engine,"validate_registry"),mock.patch.object(install.api.engine,"resolve_requested",return_value=[7]),mock.patch.object(install.api.engine,"resolve_plan",return_value=[7]),mock.patch.object(install.api.engine,"build_actions",return_value=([action],[],set())):payload,rc=install.build_payload(["7","--plan"])
 ...
 def test_install_parser_error_is_structured_without_argparse_stderr(self):
  err=io.StringIO()
  with redirect_stderr(err):payload,rc=install.build_payload(["7","--plan","--dry-run"])
 ...
 def test_restore_json_renders_structured_service_result_unchanged(self):
  ...
  with mock.patch.object(cli.restore,"plan_payload",return_value=service_payload),redirect_stdout(out):rc=cli.restore_command(["plan","/backup"],cli.CLIContext(json_output=True))
