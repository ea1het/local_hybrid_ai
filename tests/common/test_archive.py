...
class DisasterRecoveryArchiveTests(unittest.TestCase):
    ...
    def artifact(self):
        return archive.ArchiveArtifact(
            stack_id=0,
    ...
    def execute(self, root, source, when=None):
        return archive.execute_archive_backup_set(
            backup_root=root,
            source=source,
            source_commit="a" * 40,
            requested=["0"],
    ...
    def test_completed_set_has_expected_private_layout(self):
        with tempfile.TemporaryDirectory() as tmp:
            parent = Path(tmp)
            result = self.execute(self.make_root(parent), self.make_source(parent))
    ...
    def test_metadata_and_checksum_index_match_real_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(artifact["sha256"], archive.sha256_file(result.artifact_path))
            self.assertIn("backup.json", result.checksums_path.read_text())
    ...
    def test_source_files_are_not_modified(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            root = self.make_root(parent)
            before = {
                p.name: (p.stat().st_ino, stat.S_IMODE(p.stat().st_mode), hashlib.sha256(p.read_bytes()).hexdigest())
