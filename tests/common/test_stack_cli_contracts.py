...
class StackCliContractTests(unittest.TestCase):
    def test_every_manifest_stack_has_json_payload_contract(self):
        manifests = install_engine.all_manifests()
    ...
    def test_stack_contracts_return_objects_not_serialized_text(self):
        for sid, manifest in install_engine.all_manifests().items():
    ...
    def test_runtime_result_flows_through_stack_owned_contract(self):
        ...
        with mock.patch(
            "local_ai_cli.runtime_lifecycle.stack_contracts.json_payload",
            return_value={"stack": "stack7", "success": True},
        ) as builder:
            payload = result.as_dict()
        self.assertEqual(payload, {"stack": "stack7", "success": True})
