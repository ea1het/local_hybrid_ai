...
class VersionAuthorityTests(unittest.TestCase):
 ...
 def test_human_upgrade_table_calls_runtime_version_installed(self):
  ...
  self.assertIn("INSTALLED",out.getvalue().splitlines()[0])
 ...
 def test_exact_split_identity_does_not_need_registry_lookup(self):
  component=upgrade.Component("stack7","open-webui","open-webui","open-webui",None,None)
  with mock.patch.object(upgrade_adopt.upgrade_registry,"inspect") as inspect:repository,version=upgrade_adopt._split_identity(component,"ghcr.io/open-webui/open-webui:v0.11.3")
 ...
 def test_tracking_split_identity_resolves_exact_running_version(self):
  component=upgrade.Component("stack2","redis","firecrawl-redis","firecrawl-redis",None,None);state=upgrade_registry.RegistryState(image="redis:alpine",local_digest="sha256:aaa",remote_digest="sha256:bbb",current_version="8.10.0-alpine3.23",available_version="8.10.1-alpine3.23")
  with mock.patch.object(upgrade_adopt.upgrade_registry,"inspect",return_value=state):repository,version=upgrade_adopt._split_identity(component,"redis:alpine")
 ...
 def test_selection_baseline_resolves_historical_tracking_runtime(self):
  component=upgrade.Component("stack2","redis","firecrawl-redis","firecrawl-redis","stack2_-_searxng_firecrawl/docker-compose.yml","redis/redis");state=upgrade_registry.RegistryState(image="redis:alpine",local_digest="sha256:aaa",remote_digest="sha256:bbb",current_version="8.10.0-alpine3.23",available_version="8.10.1-alpine3.23")
  with mock.patch.object(upgrade_entry.upgrade,"running_image",return_value="redis:alpine"),mock.patch.object(upgrade_entry.upgrade_registry,"inspect",return_value=state):current=upgrade_entry._current_runtime_version(component,{})
 ...
 def test_selection_baseline_fails_closed_to_literal_when_registry_resolution_fails(self):
  component=upgrade.Component("stack2","redis","firecrawl-redis","firecrawl-redis","stack2_-_searxng_firecrawl/docker-compose.yml","redis/redis")
  with mock.patch.object(upgrade_entry.upgrade,"running_image",return_value="redis:alpine"),mock.patch.object(upgrade_entry.upgrade_registry,"inspect",side_effect=upgrade_registry.RegistryError("rate limited")):current=upgrade_entry._current_runtime_version(component,{})
