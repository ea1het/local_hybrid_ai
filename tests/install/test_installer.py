...
class InstallerPlannerTests(unittest.TestCase):
    def build(self, requested, plan, states, force_reconcile=False):
        with (
            patch.object(installer_impl, "stack_state", side_effect=lambda manifest: states[manifest["id"]]),
            patch.object(
                installer_impl,
                "stack_prepared",
                side_effect=lambda directory: next(
                    states[sid]["prepared"] for sid, manifest in MANIFESTS.items() if manifest["directory"] == directory
                ),
            ),
        ):
    ...
    def test_healthy_requested_consumer_only_verifies(self):
        states = {0: state(True), 6: state(True, hermes="running/healthy", **{"hermes-sandbox": "running/healthy"})}
    ...
    def test_force_reconcile_is_explicit_override(self):
        states = {0: state(True), 6: state(True, hermes="running/healthy", **{"hermes-sandbox": "running/healthy"})}
        actions, reconcile, changed = self.build([6], [0, 6], states, True)
        self.assertEqual(changed, set())
        self.assertEqual(reconcile, [6])
    ...
    def test_wait_required_runtime_accepts_starting_then_healthy(self):
        ...
        with (
            patch.object(installer_impl, "container_state", side_effect=lambda name: next(states)),
            patch.object(installer_impl.time, "sleep"),
        ):
            installer.wait_required_runtime(
                3, {"required_containers": ["litellm-postgres", "litellm"]}, timeout_seconds=10
            )

    def test_wait_required_runtime_fails_fast_on_exited_container(self):
        with patch.object(installer_impl, "container_state", return_value="exited"):
            with self.assertRaises(installer.InstallerError):
                installer.wait_required_runtime(2, {"required_containers": ["searxng"]}, timeout_seconds=10)
