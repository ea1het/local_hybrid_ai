...
class InstallerPlannerTests(unittest.TestCase):
 def build(self,requested,plan,states,force_reconcile=False):
  with patch.object(installer_impl,"stack_state",side_effect=lambda manifest:states[manifest["id"]]),patch.object(installer_impl,"stack_prepared",side_effect=lambda directory:next(states[sid]["prepared"] for sid,manifest in MANIFESTS.items() if manifest["directory"]==directory)):
 ...
 def test_wait_required_runtime_accepts_starting_then_healthy(self):
  ...
  with patch.object(installer_impl,"container_state",side_effect=lambda name:next(states)),patch.object(installer_impl.time,"sleep"):installer.wait_required_runtime(3,{"required_containers":["litellm-postgres","litellm"]},timeout_seconds=10)
