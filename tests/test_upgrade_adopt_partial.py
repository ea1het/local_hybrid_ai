...
from commands import cli,upgrade_adopt
class UpgradeAdoptPartialTests(unittest.TestCase):
 def test_unprepared_missing_component_is_skipped(self):
  ...
  with mock.patch.dict(upgrade_adopt.AUTHORITIES,{"stack1/haproxy":{"type":"split","image_key":"HAPROXY_IMAGE","version_key":"HAPROXY_VERSION"}},clear=True),mock.patch("commands.upgrade_adopt.upgrade.load_catalog",return_value=[component]),mock.patch("commands.upgrade_adopt.upgrade.running_image",return_value=None),mock.patch("commands.upgrade_adopt._component_prepared",return_value=False):updates,records=upgrade_adopt.desired_updates()
 ...
 def test_prepared_component_without_runtime_identity_fails_closed(self):
  component=SimpleNamespace(stack="stack1",name="haproxy",compose="stack1_-_haproxy_web/docker-compose.yml",container="haproxy")
  with mock.patch.dict(upgrade_adopt.AUTHORITIES,{"stack1/haproxy":{"type":"split","image_key":"HAPROXY_IMAGE","version_key":"HAPROXY_VERSION"}},clear=True),mock.patch("commands.upgrade_adopt.upgrade.load_catalog",return_value=[component]),mock.patch("commands.upgrade_adopt.upgrade.running_image",return_value=None),mock.patch("commands.upgrade_adopt._component_prepared",return_value=True):
