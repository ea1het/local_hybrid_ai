...
class RepositoryLayoutTests(unittest.TestCase):
 ...
 def test_dr_tests_do_not_shadow_production_dr_module(self):
  files=tracked_files();self.assertNotIn("tests/dr/dr.py",files);self.assertTrue(any(p.startswith("tests/disaster_recovery/") for p in files));self.assertIn("commands/backup/dr.py",files);self.assertFalse(any(p.startswith("commands/recovery/") for p in files))
