...
class RecoveryPublicCliSafetyTests(unittest.TestCase):
 ...
 def _valid_metadata(self):return {"schema_version":1,"kind":"local-hybrid-ai-backup-set","created_at":"2026-09-16T12:00:00+00:00","source_commit":"a"*40,"requested":["all"],"resolved_stacks":[0,3,6],"artifacts":[],"prerequisites":[]}
 def test_valid_metadata_fixture_is_actually_listed_completed(self):
  with tempfile.TemporaryDirectory() as tmp:self._write_set(Path(tmp),self._valid_metadata());records=cli._backup_sets(Path(tmp))
 ...
 def test_restore_drill_requires_yes_noninteractive(self):
  with mock.patch.object(cli.sys.stdin,"isatty",return_value=False),mock.patch.object(cli.recovery_api,"drill_payload") as run:rc=cli.restore_command(["drill","/backup/set","--destination","/tmp/drill"],cli.CLIContext())
  self.assertEqual(rc,2);run.assert_not_called()
 def test_restore_resume_requires_yes_noninteractive(self):
  with mock.patch.object(cli.sys.stdin,"isatty",return_value=False),mock.patch.object(cli.recovery_api,"resume_payload") as run:rc=cli.restore_command(["resume","/backup/set","--memory-sync-ssh-bootstrap","/ssh"],cli.CLIContext())
  self.assertEqual(rc,2);run.assert_not_called()
