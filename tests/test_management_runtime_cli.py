...
class ManagementRuntimeCliTests(unittest.TestCase):
 ...
 def test_internal_stack_name_is_rejected_at_public_boundary(self):
  stderr=io.StringIO()
  with contextlib.redirect_stderr(stderr),mock.patch("commands.cli.runtime_lifecycle.json_payload") as runtime:rc=cli.main(["stop","stack7"])
 ...
 def test_manifest_directory_is_rejected_at_public_boundary(self):
  stderr=io.StringIO()
  with contextlib.redirect_stderr(stderr),mock.patch("commands.cli.runtime_lifecycle.json_payload") as runtime:rc=cli.main(["stop","stack7_-_open-webui"])
  self.assertEqual(rc,2);self.assertIn("STACK_SELECTOR_INVALID",stderr.getvalue());runtime.assert_not_called()
