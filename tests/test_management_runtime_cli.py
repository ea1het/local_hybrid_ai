...
class ManagementRuntimeCliTests(unittest.TestCase):
 def test_start_dispatches_numeric_stack_selector_through_public_cli(self):
  ...
  with mock.patch.object(cli.lifecycle,"json_payload",return_value=payload) as runtime,mock.patch.object(cli.lifecycle,"cli_text",return_value="ok"),mock.patch.object(cli.render,"render_cli"):
 ...
 def test_internal_stack_name_is_rejected_at_public_boundary(self):
  stderr=io.StringIO()
  with contextlib.redirect_stderr(stderr),mock.patch.object(cli.lifecycle,"json_payload") as runtime:rc=cli.main(["stop","stack7","--yes"])
 ...
 def test_manifest_directory_is_rejected_at_public_boundary(self):
  stderr=io.StringIO()
  with contextlib.redirect_stderr(stderr),mock.patch.object(cli.lifecycle,"json_payload") as runtime:rc=cli.main(["stop","stack7_-_open-webui","--yes"])
  self.assertEqual(rc,2);self.assertIn("STACK_SELECTOR_INVALID",stderr.getvalue());runtime.assert_not_called()
 def test_json_flag_is_owned_by_public_renderer(self):
  payload={"success":True,"command":"runtime.start","stack":"stack7","directory":"stack7_-_open-webui","containers":["open-webui"]}
  with mock.patch.object(cli.lifecycle,"json_payload",return_value=payload) as runtime,mock.patch.object(cli.render,"render_json") as renderer:
 ...
 def test_runtime_mutation_noninteractive_requires_yes(self):
  with mock.patch.object(cli.lifecycle,"json_payload") as runtime:
   rc=cli.main(["start","7"])
  self.assertEqual(rc,2);runtime.assert_not_called()
 def test_runtime_mutation_json_requires_yes(self):
  out=io.StringIO()
  with contextlib.redirect_stdout(out),mock.patch.object(cli.lifecycle,"json_payload") as runtime:
