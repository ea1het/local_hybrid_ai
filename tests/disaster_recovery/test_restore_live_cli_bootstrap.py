...
"""Protect Stack6 SSH bootstrap handling in the importable recovery service.

The public management boundary is ``./local-ai``.  These tests exercise the
service implementation directly; they must not turn the legacy restore-live.py
wrapper into a second Python/CLI contract.
"""
...
from commands.recovery import dr_restore_live_service as restore_service
...
class RestoreLiveBootstrapTests(unittest.TestCase):
    ...
    def test_global_operational_env_uses_resource_id_contract(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            metadata = {"global_artifacts": [{"resource_id": "operational-env", "relative_path": "artifacts/global/operational.env"}]}
    ...
    def test_global_operational_env_rejects_wrong_field_name(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with self.assertRaises(restore_service.dr_restore_live.RestoreLiveError):
    ...
    def test_install_bootstrap_copies_private_material_to_clean_runtime_target(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            base = root / "runtime"
            values = {"BASE_PATH": str(base), "MEMORY_SYNC_SERVICE": "service_-_hermes-memory-sync", "HERMES_UID": "1000", "HERMES_GID": "1000"}
            with mock.patch.object(restore_service.dr_restore_all, "read_completed_backup_set", return_value={}), \
                 mock.patch.object(restore_service, "read_env_artifact", return_value=(root / "env", values)), \
                 mock.patch.object(restore_service.dr_restore_live, "_absolute_safe_path", return_value=base), \
    ...
    def test_enable_memory_sync_starts_profile_and_requires_running_container(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch.object(restore_service.dr_restore_all, "read_completed_backup_set", return_value={}), \
                 mock.patch.object(restore_service, "read_env_artifact", return_value=(root / "env", values)), \
                 mock.patch.object(restore_service.dr_restore_compat, "wait_required_runtime") as wait_ready, \
            ...
            self.assertEqual(run.call_args_list[1].args[0][-1], "hermes-memory-sync")
