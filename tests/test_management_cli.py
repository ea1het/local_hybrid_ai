...
class ManagementCliContractTests(unittest.TestCase):
    ...
    def test_completion_restore_actions_include_backup_listing(self):
        ...
        for item in ("apply", "drill", "list-backup-sets", "plan", "resume"):
            self.assertIn(item, values)

    def test_completion_upgrade_stack_selector_is_numeric(self):
        values = completion.complete(["upgrade", ""])
        for item in [str(i) for i in range(8)] + ["adopt", "check", "--offline", "policy", "--yes", "--json"]:
            self.assertIn(item, values)
    ...
    def test_completion_upgrade_adopt_has_no_stack_argument(self):
        ...
        self.assertEqual([v for v in values if not v.startswith("--")], [])
    ...
    def test_completion_rejects_unknown_shell(self):
        self.assertNotEqual(self.run_cli("completion", "fish").returncode, 0)
    ...
    def test_backup_destination_reaches_structured_api(self):
        ...
        self.assertEqual(rc, 0)
        run.assert_called_once_with("/mnt/backup/local-ai")
    ...
    def test_json_backup_yes_is_rendered_by_public_cli(self):
        ...
        with (
            mock.patch.object(cli.backup, "backup_payload", return_value=payload) as run,
            mock.patch.object(cli.render, "render_json") as rendered,
        ):
            rc = cli.main(["--json", "backup", "--destination", "/mnt/backup/local-ai", "--yes"])
        self.assertEqual(rc, 0)
        run.assert_called_once_with("/mnt/backup/local-ai")
        rendered.assert_called_once_with(payload)

    def test_restore_public_grammar_dispatches_to_structured_api(self):
        cases = (
            (["plan", "/backup/set"], "plan_payload", ("/backup/set",), False),
            (
                ["drill", "/backup/set", "--destination", "/tmp/drill"],
                "drill_payload",
                ("/backup/set", "/tmp/drill"),
                True,
            ),
            (["apply", "/backup/set", "--check-clean-target"], "check_clean_target_payload", ("/backup/set",), False),
            (
        ...
        for public_args, name, expected, needs_yes in cases:
            ...
            with (
                ...
                mock.patch.object(cli.restore, name, return_value=payload) as run,
                mock.patch.object(cli.render, "render_cli"),
            ):
                rc = cli.restore_command(public_args, cli.CLIContext(assume_yes=needs_yes))
                self.assertEqual(rc, 0)
    ...
    def test_restore_without_action_shows_public_restore_help(self):
        ...
        self.assertEqual(cp.returncode, 0, cp.stderr)
        for item in ("list-backup-sets", "plan", "drill", "apply", "resume"):
            self.assertIn(item, cp.stdout)

    def test_json_list_backup_sets_has_public_versioned_contract(self):
        with tempfile.TemporaryDirectory() as tmp:
            cp = self.run_cli("restore", "list-backup-sets", "--json", env_extra={"DR_BACKUP_ROOT": tmp})
        self.assertEqual(cp.returncode, 0, cp.stderr)
        payload = json.loads(cp.stdout)
