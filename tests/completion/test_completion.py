...
class CompletionTests(unittest.TestCase):
 ...
 def test_stack_completion_is_package_local(self):
  with mock.patch("local_ai_cli.completion.api.manifests.all_manifests",return_value={0:{},2:{},7:{}}):self.assertEqual([v for v in completion.complete(["start",""]) if not v.startswith("--")],["0","2","7"])
 def test_install_completion_exposes_public_options(self):
  with mock.patch("local_ai_cli.completion.api.manifests.all_manifests",return_value={0:{},7:{}}):values=completion.complete(["install",""])
 ...
 def test_upgrade_components_are_package_local(self):
  with mock.patch("local_ai_cli.completion.api.manifests.all_manifests",return_value={2:{}}),mock.patch("local_ai_cli.completion.api.upgrade_components.upgrade_components",return_value=["redis","searxng"]):self.assertEqual([v for v in completion.complete(["upgrade","2",""]) if not v.startswith("--")],["redis","searxng"])
