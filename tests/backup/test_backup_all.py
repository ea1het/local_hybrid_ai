...
class BackupAllTests(unittest.TestCase):
    ...
    def test_rejects_lifecycle_container_not_owned_by_manifest(self):
        ...
        lifecycle["stacks"]["1"]["required_containers"] = ["foreign"]
    ...
    def test_backup_destination_ancestor_of_protected_root_is_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            protected_parent = root / "docker"
            protected_parent.mkdir()
            stacks = protected_parent / "stacks"
            runtime = protected_parent / "runtime"
            stacks.mkdir()
            runtime.mkdir()
            with self.assertRaises(backup_all.BackupAllError):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                ...
                with self.assertRaisesRegex(
                    backup_all.BackupAllError, "unsafe backup destination overlaps STACKS_ROOT"
                ):
