...
class Stack3BackupAdapterTests(unittest.TestCase):
    def test_expected_dependency_complete_artifact_paths(self):
        self.assertEqual(
            stack3_backup.PKI_RELATIVE_PATH,
            "artifacts/stack0/platform-pki.tar",
        )
        self.assertEqual(
            stack3_backup.DB_RELATIVE_PATH,
    ...
    def test_select_resources_requires_stack0_and_stack3_plan(self):
        ...
        pki, database, salt = stack3_backup.select_resources(manifests, [0, 3])
        self.assertEqual(pki["id"], "platform-pki")
        self.assertEqual(database["id"], "litellm-database")
        self.assertEqual(salt["id"], "litellm-salt")
        with self.assertRaises(stack3_backup.Stack3BackupError):
            stack3_backup.select_resources(manifests, [3])
    ...
    def test_restore_checksum_verifier_rejects_tampering(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(stack3_restore_verify.Stack3RestoreVerifyError):
                stack3_restore_verify.verify_checksums(root, metadata)
    ...
    def test_database_artifact_selector_is_exact(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                stack3_restore_verify.select_database_artifact(root, metadata),
                path,
            )
