...
class DisasterRecoveryPlannerTests(unittest.TestCase):
    ...
    def test_backup_plan_classifies_required_and_external_prerequisites(self):
        ...
        self.assertEqual(
            {(i.stack_id, i.resource_id): i.kind for i in prerequisites},
            {
                (3, "litellm-salt"): "REQUIRE",
                (6, "hermes-knowledge"): "EXTERNAL",
                (7, "open-webui-secret-key"): "REQUIRE",
    ...
    def test_backup_destination_cli_overrides_environment(self):
        path, source = dr_planner.resolve_backup_root("/mnt/backup/local-ai", environ={"DR_BACKUP_ROOT": "/srv/dr"})
    ...
    def test_gitea_help_flags_are_parsed_without_help_text_storage(self):
        self.assertEqual(
            dr_planner.gitea_help_flags("Usage: gitea dump --file value --tempdir value --skip-repository"),
            ["--file", "--skip-repository", "--tempdir"],
        )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            manifests = {
                        ...
                        "resources": [
                            {
                                ...
                                "strategy": "archive",
                                "config": {
                                    "source": {"type": "runtime-path", "path": "${BASE_PATH}/service_-_platform/pki"}
                                },
                            }
                        ]
                    },
                    ...
                    "recovery": {
                        "resources": [
                            ...
                            },
                            {
                                ...
                                "config": {"source": {"type": "environment", "key": "SALT"}},
                            },
                        ]
                    },
                },
                ...
                6: {
                    "owns": [],
                    "recovery": {
                        "resources": [{"id": "knowledge", "strategy": "git", "config": {"source": {"type": "git"}}}]
                    },
                },
            }
            ...
            checks = dr_planner.preflight_runtime_sources(
                manifests, [0, 3, 4, 6], env_path=env_path, runner=fake_runner, docker_available=True
            )
            serialized = repr([c.as_dict() for c in checks])
            self.assertNotIn("super-secret-value", serialized)
            self.assertIn("Gitea version 1.24.6", serialized)
            self.assertIn("--file", serialized)
            self.assertTrue(any(c.check == "postgres-source" for c in checks))
            self.assertTrue(any(c.status == "DECLARED" for c in checks))
    ...
    def test_runtime_preflight_rejects_unowned_service(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(dr_planner.RecoveryError):
                dr_planner.preflight_runtime_sources(manifests, [3], env_path=env_path, docker_available=True)
    ...
    def test_completed_backup_schema_is_distinct_from_dry_run_plan(self):
        schema_path = ROOT.parent / "common" / "backup-set.schema.json"
        self.assertTrue(schema_path.is_file())
        text = schema_path.read_text(encoding="utf-8")
        self.assertIn('"local-hybrid-ai-backup-set"', text)
        self.assertIn('"sha256"', text)
        self.assertIn('"size_bytes"', text)
        self.assertNotIn('"local-hybrid-ai-backup-plan"', text)
