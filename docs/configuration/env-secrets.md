...
- If authentication is introduced, define provider-side issuance and validation first and document the new lifecycle here.
...
For DR, the platform PKI is a **managed persistent-identity resource** and must be preserved so a rebuilt platform can retain the existing trust chain. See [the DR how-to](../dr/howto.md).
...
Every new stack must also declare its recovery semantics under the normalized contract described by [`recovery.schema.json`](../../commands/recovery/recovery.schema.json) and [the DR how-to](../dr/howto.md) before its backup/restore behavior is implemented.
