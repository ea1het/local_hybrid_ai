...
A target must be selected explicitly before `upgrade --yes` can mutate runtime. Compatibility policy and selectable classification are separate gates. Selectable classification defaults from the manifest but is not fixed at install time: exactly three components (`platform-foundation`, Stack4 `runner`, Stack6 `sandbox`) are non-selectable by default; every other component is selectable by default. An administrator can override the effective classification in either direction with a persistent, installation-owned override (`upgrade selectable`), independent from the Git-tracked manifest. An override changes only the classification, not target/digest validation, compatibility, stale-plan checks, recovery, READY/VERIFY or consumer checks, and a component still needs a manifest-declared mutation recipe before apply will act on it.

Component upgrade mutation follows one of two recipes declared per component: a generic `env-version` recipe (swap one or two operational `.env` keys, rerun the stack's targeted deploy command) or, where a version bump is not safely reversible with a simple restart, a dedicated recipe. PostgreSQL major-version upgrades use such a dedicated recipe: it dumps the running database, stops the dependent application and the database, moves the old data directory aside (never deleted automatically), starts the new image against a fresh data directory, restores the dump, verifies the table inventory matches, and only then resumes the dependent application. Applying a selection that uses this recipe requires `--confirm-data-migration` in addition to `--yes`, and it must be applied in isolation from any other selection.

The upgrade implementation keeps command orchestration and payload construction (`api.py`), domain configuration/catalog/inventory (`config.py`) and guarded mutation (`engine.py`) as the package's public modules, with policy, selection validation, registry discovery and the PostgreSQL major-version recipe as private implementation modules behind them. Runtime image/version and OCI identity primitives are shared instead of being reimplemented inside the executor. These are internal maintainability boundaries; external consumers continue to use `./local-ai`.
