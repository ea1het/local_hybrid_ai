...
- Runtime-qualify fail-closed registry error behavior for rate-limit/authentication failures. Positive-path tag/digest mapping across Docker Hub, GHCR and `docker.gitea.com`, including digest-only Firecrawl pins, is qualified on m92p.
...
Registry-native positive-path qualification is now broader than the original mutable-tag gate. Live m92p `upgrade check` resolved Docker Hub packages, GHCR packages and `docker.gitea.com/gitea` from their configured registry/repository, preserved full local/remote digests, mapped Firecrawl digest-only pins to operator-facing versions where possible, kept latest-only digest packages current when their immutable identity matched, and reported coherent update availability without lateral GitHub Release discovery. Runtime qualification of explicit 429/authentication failure states remains open.
