...
The operator UX has moved into guarded execution: `local-ai` is the anticorruption boundary, the installation-local upgrade plan exposes Current/Available/Selected state, stale selections fail closed, exact target images are preflighted read-only, and the executor is recovery-first where durable state requires it. Stack6 remains reconstructable; only Git-backed `MEMORY.md` + `USER.md` is durable user memory, so Hermes sessions/SQLite/cache state are not upgrade recovery requirements. Real live upgrade qualification is still pending and must be performed deliberately; implementation alone is not evidence of a successful production upgrade.
